...
foreach($probe in $p.probes){
    ...
    $sddl=$null
    try{$sddl=(Get-Acl -LiteralPath $probe.path).Sddl}catch{}
    $rows += [ordered]@{path=$probe.path;right=$probe.right;mask=$probe.mask;expected=$probe.expected;result=$result;win32_error=$code;pass=$pass;sddl_if_readable=$sddl}
}
$identityMatches=($identity.User.Value -ceq $p.sid)
$admin=$principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)
if(-not $identityMatches -or $admin){$failed=$true}
$result=[ordered]@{schema='imperium.local-access-measurement/v1';role=$p.role;sid=$identity.User.Value;expected_sid=$p.sid;identity_matches=$identityMatches;administrator_token=$admin;groups=@($identity.Groups|ForEach-Object{$_.Value});utc=[DateTimeOffset]::UtcNow.ToString('o');plan_sha256=(Get-FileHash -LiteralPath $Plan).Hash;probes=$rows;result=if($failed){'ISOLATION_FAILED_OR_UNRESOLVED'}else{'RECORDED_ACCESS_EXPECTATIONS_MET'};deployment_isolation_claimed=$false}
