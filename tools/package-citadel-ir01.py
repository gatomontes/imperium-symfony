...
def main():
    ...
    if verification['runs']['full-suite']['exit_code'] != 0:
    ...
    fixed = {'composer.json', 'composer.lock', 'symfony.lock', 'imperium-doctrine.md', 'bin/console',
             ...
             'tools/prove-citadel-readiness.php', 'tools/package-citadel-readiness.py',
             'tools/prove-citadel-ir01.php', 'tools/verify-citadel-ir01.py', 'tools/package-citadel-ir01.py', 'tools/verify-citadel-correction-proof.py'}
    ...
    proof_files = ['entry-identity.json', 'focused.txt', 'focused.xml', 'container-lint.txt',
                   'ir01-post-correction.json', 'readiness-demo.json', 'offline-demo.json',
                   'correction-demo.json', 'full-suite.txt', 'full-suite.xml', 'verification.json',
                   'preservation-check.json', 'generated-reference.diff']
    proof_files += [name + '.stderr.txt' for name in ['focused', 'container-lint', 'ir01-post-correction',
                    'readiness-demo', 'offline-demo', 'correction-demo', 'full-suite']]
    ...
    post = json.loads((PROOF / 'ir01-post-correction.json').read_text())
    if post['extra_provider_calls_after_understood'] != 0 or not post['journal_and_exposure_unchanged']:
        raise SystemExit('IR01 regression')
    bundle = ROOT / 'var/citadel-ir01-review-history.bundle'
    if bundle.exists(): raise SystemExit('Preserve existing bundle')
    subprocess.check_call(['git', 'bundle', 'create', str(bundle), 'HEAD', '^' + BASE], cwd=ROOT)
    subprocess.check_call(['git', 'bundle', 'verify', str(bundle)], cwd=ROOT)
    entries['history.bundle'] = bundle.read_bytes()
    for filename in ['citadel-readiness-independent-review.md', 'citadel-readiness-ir01-review-addendum.md']:
        entries['review/' + filename] = (ROOT / filename).read_bytes()
    entries['proof/ir01-pre-correction.json'] = (ROOT / 'var/citadel-readiness-ir01-diagnostic.json').read_bytes()
    ...
    archive = out / ('citadel-ir01-' + head[:12] + '.zip')
