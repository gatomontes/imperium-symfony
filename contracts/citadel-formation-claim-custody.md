...
| Response metadata | Validate actual result and usage; commit RESPONSE_VALIDATED_PENDING_ENVELOPE with exact response SHA identity, provider ID, usage and provenance | Seal immutable envelope under separate immutable-store lock | A dispatch fence alone cannot authenticate a body; missing envelope remains unrecoverable with full exposure |
| Response completion | Commit RESPONSE_RETAINED with envelope digest after matching retained response identity | Return actual result | A crash after envelope publication retains its original provider ID/provenance and exact body binding; recovery needs both validated custody metadata and the matching trusted-root envelope |
