...
final class CanonicalNativeEffectReconciliationAuthorityProvenanceRemediationBatch4Test extends CanonicalNativeEffectCorridorActivationBatch4Test
{
    public function testIssuerInterruptionLeavesAnUnresolvableOrphanAndExactRetryCompletesEvidence(): void
    {
        ...
        $this->fails('synthetic issuance cut', fn () => Support\ReconciliationAuthorityFixture::issue($this->state, $admission['admission_id'], $at + 1, $at + 100, static function (string $cut): void {
        ...
        $retried = Support\ReconciliationAuthorityFixture::issue($this->state, $admission['admission_id'], $at + 1, $at + 100);
    ...
    {
        [$admission, $at] = $this->sealedResponse();
        $issued = Support\ReconciliationAuthorityFixture::issue($this->state, $admission['admission_id'], $at + 1, $at + 100);
    ...
    {
        $issued = Support\ReconciliationAuthorityFixture::issue($this->state, $admission['admission_id'], $at, $expiresAt);
        $resolver = new NativeEffectReconciliationAuthorityResolver($this->state);
