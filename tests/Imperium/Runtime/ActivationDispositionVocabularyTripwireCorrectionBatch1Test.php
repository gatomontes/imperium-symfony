...
/** Mutations call the actual corrected coverage detector on disposable source trees. */
final class ActivationDispositionVocabularyTripwireCorrectionBatch1Test extends TestCase
{
    ...
    public function testLexicalMatrixProvesCorrectedDetector(): void
    {
        ...
        foreach (self::VALUES as $value) {
            foreach ($cases as $case) {
                ...
                $this->assertDetector(in_array($case['batch1_classification'], ['EXACT_LITERAL', 'STATIC_MULTILINE', 'EXACT_LITERAL_COMPONENT', 'EXACT_LITERAL_DEFINITION', 'EXACT_LITERAL_NON_DECISION'], true), $value.':'.$case['id']);
            }
        }
    }
    ...
    public function testExactPathAdmissionRemovalAndRoleLimitations(): void
    {
        ...
        $path = $this->root.'/'.$fields[1];
        file_put_contents($path, "<?php\n// 'RETIRE_CORRIDOR'\n");
        $this->assertDetector(true, 'comment cannot preserve an admitted path');
        file_put_contents($path, "<?php\nconst VALUE = 'RETIRE_CORRIDOR';\n");
    ...
    public function testAdditionalLiteralAndNonProducerBoundaries(): void
    {
        $cases = json_decode((string) file_get_contents(dirname(__DIR__, 3).'/docs/activation-disposition-vocabulary-tripwire-correction-batch-1-cases-v1.json'), true, flags: JSON_THROW_ON_ERROR);
        self::assertCount(30, $cases);
        foreach (self::VALUES as $value) {
            foreach ($cases as $case) {
                $source = "<?php\n".$this->expand($case['source'], $value)."\n";
                token_get_all($source, TOKEN_PARSE);
                file_put_contents($this->root.'/src/Imperium/Runtime/PreparationSynthetic.php', $source);
                $this->assertDetector($case['rejects'], $value.':'.$case['id']);
            }
        }
    }

    public function testAllSixPathsSurviveQuoteConversionAndRejectSameCountSubstitution(): void
    {
        $rows = file($this->root.'/'.self::INVENTORY, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
        foreach ($rows as $row) {
            if (str_starts_with($row, '#') || str_starts_with($row, "classification\t")) {
                continue;
            }
            $path = $this->root.'/'.explode("\t", $row)[1];
            $source = (string) file_get_contents($path);
            foreach (self::VALUES as $value) {
                $source = str_replace("'".$value."'", '"'.$value.'"', $source);
            }
            file_put_contents($path, $source);
            $this->assertDetector(false, 'quote conversion '.$path);
        }
        $replacement = $this->root.'/src/Imperium/Runtime/PreparationSynthetic.php';
        rename($path, $replacement);
        $this->assertDetector(true, 'same count, different path');
        rename($replacement, $path);
        $this->assertDetector(false, 'same bytes at restored admitted path');
    }

    public function testMalformedSyntaxFailsWithPathAndLine(): void
    {
        $path = $this->root.'/src/Imperium/Runtime/PreparationSynthetic.php';
        foreach (["<?php\n\$x = ;\n", "<?php\n\$x = <<<TEXT\nRETIRE_CORRIDOR\n"] as $source) {
            file_put_contents($path, $source);
            $message = $this->assertDetector(true, 'malformed source');
            self::assertStringContainsString('PreparationSynthetic.php:', $message);
            self::assertMatchesRegularExpression('/\.php:\d+:/', $message);
        }
    }

    public function testRepositoryFallbackAndOverrideRestoration(): void
    {
        $override = getenv('IMPERIUM_FROZEN_COVERAGE_ROOT');
        try {
            foreach ([false, ''] as $fallback) {
                putenv(false === $fallback ? 'IMPERIUM_FROZEN_COVERAGE_ROOT' : 'IMPERIUM_FROZEN_COVERAGE_ROOT=');
                $this->assertDetector(false, 'repository fallback');
            }
        } finally {
            putenv('IMPERIUM_FROZEN_COVERAGE_ROOT='.$override);
        }
        self::assertSame($this->root, getenv('IMPERIUM_FROZEN_COVERAGE_ROOT'));
        file_put_contents($this->root.'/src/Imperium/Runtime/PreparationSynthetic.php', "<?php\nconst VALUE = 'RETIRE_CORRIDOR';\n");
        $this->assertDetector(true, 'restored override uses disposable source');
        self::assertSame($this->root, getenv('IMPERIUM_FROZEN_COVERAGE_ROOT'));
    }

    private function assertDetector(bool $expectedFailure, string $label): string
    {
        $failed = false;
        $message = '';
        try {
    ...
    private function expand(string $source, string $value): string
    {
        return strtr($source, [
            '{{V}}' => $value,
            '{{FIRST_HEX}}' => sprintf('\\x%02x', ord($value[0])),
            '{{FIRST_OCT}}' => sprintf('\\%03o', ord($value[0])),
            '{{REST}}' => substr($value, 1),
