...
final class IronGateEvidenceAuthenticityRemediationBatch9Test extends TestCase
{
    public function testThreeTransitionsRequireSeparateCallerAuthorityIdentity(): void
    {
        ...
        foreach(['all three deterministic outbound-email transitions','consumed native Imperator principal authority','consumption-before-target crash gap','Only Batch 10 may next be considered','Batch 10 is not authorized','No provider callback'] as $proof) self::assertStringContainsString($proof,$handoff);
    }
}
