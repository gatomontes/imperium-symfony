...
final class ConstableSeatBindingServiceTest extends TestCase
{
    ...
    {
        $root = sys_get_temp_dir().'/imperium-constable-binding-'.bin2hex(random_bytes(6)); $project = dirname(__DIR__, 3);
        ...
        $registry = new CanonicalConstableRegistry($project); $substrate = new GenericOfficerSubstrateRegistry($root); $member = $registry->member();
        $deliveryId = 'qualified-delivery-1234567890abcdef1234'; $manifestationId = 'imperium-test.officer.garrison.constable.123456789abc';
        $qualification = ['candidate_id' => $manifestationId, 'qualification_contract' => $member['qualification_contract'], 'disposition' => 'QUALIFIED'];
        $packet = ['schema' => 'imperium.qualified-manifestation-packet/v1', 'delivery_id' => $deliveryId, 'commission' => ['consumed' => true],
            'candidate' => ['manifestation_id' => $manifestationId, 'instance_id' => 'imperium-test', 'persona' => $member['persona'], 'profile' => $member['profile'],
                'substrate_instance' => ['substrate' => $substrate->current(), 'status' => 'PROFILE_INSTALLED'], 'target_seat' => 'garrison.constable', 'target_occupancy_generation' => 1, 'status' => 'QUALIFIED_UNBOUND'],
        ...
        mkdir($root.'/var/imperium/mastermason/qualified-manifestations', 0770, true); file_put_contents($root.'/var/imperium/mastermason/qualified-manifestations/'.$deliveryId.'.json', json_encode($packet, JSON_THROW_ON_ERROR));
        try {
            $service = new ConstableSeatBindingService($root, $bootstrap, $registry, $substrate); $occupancy = $service->bind($deliveryId);
            self::assertSame($occupancy, $service->bind($deliveryId)); self::assertSame('imperium.garrison-constable-occupancy/v1', $occupancy['schema']);
            self::assertSame('garrison.constable', $occupancy['seat']); self::assertSame($manifestationId, $occupancy['manifestation_id']); self::assertSame('ACTIVE', $occupancy['status']);
            ...
            self::assertTrue($occupancy['inventory_response_authority']); self::assertFalse($occupancy['selection_authority']); self::assertFalse($occupancy['execution_authority']);
            self::assertFileExists($root.'/var/imperium/offices/garrison/occupancy/'.$occupancy['binding_id'].'.json');
