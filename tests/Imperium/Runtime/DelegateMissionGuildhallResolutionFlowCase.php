...
use App\Imperium\Runtime\Garrison\DelegateMissionTerminalReturnService;
...
abstract class DelegateMissionGuildhallResolutionFlowCase extends TestCase
{
    ...
    public function testDelegateModelCriteriaAreAuthorizedBeforeExactOracleCommissionIssuance(): void
    {
        ...
        try {
            ...
            $resultDisposition=(new DelegateMissionCognitionResultDispositionService($root))->decide($turn['turn_id'],$turn['curia_result_disposition_authority']['authority_id'],$seneschal,'ACCEPTED','Accept the completed bounded result and proceed only to governed return.',$issued->modify('+12 minutes'));self::assertSame('DELEGATE_MISSION_RESULT_DISPOSED_PENDING_RETURN_AUTHORIZATION',$resultDisposition['status']);self::assertTrue($resultDisposition['return_authorization_authority']['authority_exercisable']);
            $return=(new DelegateMissionReturnAuthorizationService($root))->authorize($resultDisposition['disposition_id'],$resultDisposition['return_authorization_authority']['authority_id'],$seneschal,$issued->modify('+13 minutes'));self::assertSame('DELEGATE_MISSION_RETURN_AUTHORIZED_PENDING_GARRISON_TERMINAL_TRANSITION',$return['status']);self::assertTrue($return['garrison_terminal_return_authority']['authority_exercisable']);
            $terminal=(new DelegateMissionTerminalReturnService($root))->complete($return['authorization_id'],$return['garrison_terminal_return_authority']['authority_id'],'garrison-constable-binding-'.str_repeat('7',20),$issued->modify('+14 minutes'));self::assertSame('DELEGATE_MISSION_RETURNED_UNBOUND_CUSTODY_RESTORED_RETIRED_TERMINAL',$terminal['status']);self::assertTrue($terminal['returned']);self::assertTrue($terminal['custody_restored']);self::assertTrue($terminal['manifestation_retired']);self::assertFalse($terminal['seat_bound']);foreach(['operational_use_authority','provider_invocation_authority','credential_use_authority','tool_use_authority','perimeter_crossing_authority','external_action_authority','execution_authority','continuing_authority','redeployment_authority','reuse_authority']as$field)self::assertFalse($terminal[$field]);$audit=(new DelegateMissionOperationalEvidenceAuditService($root))->audit($terminal['terminal_id']);self::assertSame('DELEGATE_MISSION_OPERATIONAL_EVIDENCE_VALID',$audit['status']);self::assertSame('FOURTEEN_RECORD_OPERATIONAL_SUBCHAIN_ONLY',$audit['completeness_claim']);self::assertSame(14,$audit['verified_records']);self::assertFalse($audit['continuing_authority']);$auditor=new DelegateMissionOperationalEvidenceAuditService($root);$returnPath=$root.'/var/imperium/offices/curia/delegate-mission-return-authorizations/'.$return['authorization_id'].'.json';unlink($returnPath);try{$auditor->audit($terminal['terminal_id']);self::fail('Missing operational evidence must fail stopped.');}catch(\RuntimeException$e){self::assertSame('AUD302_DELEGATE_RETURN_AUTHORIZATION_INVALID',$e->getMessage());}$this->write($returnPath,$return);$terminalPath=$root.'/var/imperium/offices/garrison/delegate-mission-terminal-returns/'.$terminal['terminal_id'].'.json';$tampered=$terminal;$tampered['status']='TAMPERED';$this->write($terminalPath,$tampered);try{$auditor->audit($terminal['terminal_id']);self::fail('Digest tampering must fail stopped.');}catch(\RuntimeException$e){self::assertSame('AUD301_DELEGATE_TERMINAL_ABSENT',$e->getMessage());}$this->write($terminalPath,$terminal);$stale=$terminal;unset($stale['record_digest']);$stale['source_return_authorization']['digest']=str_repeat('0',64);$this->write($terminalPath,$this->record($stale));try{$auditor->audit($terminal['terminal_id']);self::fail('A stale reference must fail stopped.');}catch(\RuntimeException$e){self::assertSame('AUD302_DELEGATE_RETURN_AUTHORIZATION_INVALID',$e->getMessage());}$this->write($terminalPath,$terminal);$substituteId='delegate-mission-return-authorization-'.str_repeat('f',20);$this->write($root.'/var/imperium/offices/curia/delegate-mission-return-authorizations/'.$substituteId.'.json',$return);$substituted=$terminal;unset($substituted['record_digest']);$substituted['source_return_authorization']=['id'=>$substituteId,'digest'=>$return['record_digest']];$this->write($terminalPath,$this->record($substituted));try{$auditor->audit($terminal['terminal_id']);self::fail('Reference substitution must fail stopped.');}catch(\RuntimeException$e){self::assertSame('AUD302_DELEGATE_RETURN_AUTHORIZATION_INVALID',$e->getMessage());}$this->write($terminalPath,$terminal);
        } finally {$this->remove($root);}
    }
    ...
    private function reservationRequestFixture(bool $available = true): array
    {
        ...
        $this->write($root.'/var/imperium/offices/garrison/occupancy/'.$constableBindingId.'.json', $this->record([
            ...
            'delegate_mission_terminal_return_authority' => true,
