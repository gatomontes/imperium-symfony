...
final class ConstableConscriptionServiceTest extends TestCase
{
    public function testConsumesExactCommissionAndReturnsQualifiedUnboundConstable(): void
    {
        $root = sys_get_temp_dir().'/imperium-constable-conscription-'.bin2hex(random_bytes(6)); $project = dirname(__DIR__, 3);
        $bootstrap = new StateStore($root); $registry = new CanonicalConstableRegistry($project); $substrate = new GenericOfficerSubstrateRegistry($root);
        ...
        $case = ['case_id' => $caseId, 'instance_id' => 'imperium-test', 'status' => 'CANONICAL_CONSTABLE_READY']; $case['record_digest'] = hash('sha256', CanonicalJson::encode($case));
        ...
        $member = $registry->member(); $commissionId = 'constable-construction-1234567890abcdef1234';
        $commission = ['schema' => 'imperium.construction-commission/v1', 'commission_id' => $commissionId, 'issuer' => 'mastermason',
            'source_provisioning_case_id' => $caseId, 'source_provisioning_case_digest' => $case['record_digest'], 'instance_id' => 'imperium-test', 'office' => 'garrison', 'target_seat' => 'garrison.constable',
            ...
            'status' => 'ISSUED_PENDING_CONSCRIPTION', 'spawning_authority' => true, 'seat_binding_authority' => false, 'inventory_response_authority' => false, 'execution_authority' => false];
        ...
        try {
            $service = new ConstableConscriptionService($root, $bootstrap, $registry, $substrate); $result = $service->fulfill($commissionId); $delivery = $result['delivery'];
            self::assertSame($result, $service->fulfill($commissionId));
            self::assertSame('imperium-test.officer.ordinary-recruiter.1', $result['recruiter']['manifestation_id']);
            self::assertSame('garrison.constable', $delivery['candidate']['target_seat']); self::assertSame('QUALIFIED_UNBOUND', $delivery['candidate']['status']);
            ...
            self::assertTrue($delivery['sealed']); self::assertFalse($delivery['seat_binding_authority']); self::assertFalse($delivery['inventory_response_authority']); self::assertFalse($delivery['execution_authority']);
