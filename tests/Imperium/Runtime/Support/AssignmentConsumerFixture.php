...
final class AssignmentConsumerFixture
{
    public AugurFreshFixture $fresh;
    public AssignmentFormationFixture $formation;
    ...
    public function __construct(public string $mode='valid',public string $applicationMode='A', public string $instance='instance-test')
    {
        $pins=[];
        $this->fresh=new AugurFreshFixture(configure:function(array &$p,OnboardingAuthorityFixture $f,array $profile)use(&$pins):void{
            ...
            $this->formation=new AssignmentFormationFixture($f->root,$f->now,$this->instance);
            $this->formation->appoint();
            $state=$this->formation->journal->read()['state'];
            foreach($p['body']['targets'] as &$target){
                $holder=$state[$target['role']==='courtyard.courtthane'?'courtthane':'locksmith'];
                $target['profile_ref']=$f->source('formation-profile-mapping',[
                    'schema'=>'imperium.formation-profile-mapping/v1','instance_id'=>$this->instance,
                    'formation_citadel_id'=>$state['citadel_id'],'seat'=>$target['role'],'holder_generation'=>$holder['generation'],
            ...
            $target=$p['body']['targets'][0];$binding=$p['body']['candidate_bindings'][0];
            $tuple=['role'=>$target['role'],'provider'=>$binding['provider'],'model_id'=>$binding['model_id'],'model_version'=>$binding['model_version'],
                'binding_ref'=>$binding['binding_ref'],'configuration_ref'=>$binding['configuration_ref'],'profile_ref'=>$target['profile_ref'],
                'profile_generation'=>1,'binding_generation'=>1,'generation'=>1];
            foreach(['ordinary','prepared','stale','stale-prepared','custody-race'] as $name){
                $intake=$this->formation->receive('Bounded synthetic '.$name,'consumer-request-'.$name)['intake_id'];
                $terms=$this->formation->terms($intake,'interview');
                $terms['provider']=$tuple['provider'];$terms['model']=$tuple['model_id'];$terms['model_settings']=$tuple;
                $terms['per_call']['output_tokens']=4096;
                if(in_array($name,['prepared','stale-prepared','custody-race'],true)){$terms['transport']=FormationCustodyFixture::authorization();}
                $this->sessions[$name]=$this->formation->grant($intake,'interview',$terms);
            }
            $state=$this->formation->journal->read()['state'];
            $budget=Policy::content($f->sources[R::key($p['body']['budget_ref'])],'shared-budget');
            foreach($this->sessions as $sid){$budget['formation_sources'][]=['session_id'=>$sid,
                'resource_decision_digest'=>\App\Imperium\Runtime\Citadel\Formation\FormationJournal::digest($state['sessions'][$sid]['provider_resource_decision'])];}
            $p['body']['budget_ref']=$f->source('shared-budget',$budget);
    ...
    public function evidence():\App\Imperium\Runtime\Onboarding\Assignment\AssignmentEvidence
    {
        return new class($this) implements \App\Imperium\Runtime\Onboarding\Assignment\AssignmentEvidence {
            public function __construct(private AssignmentConsumerFixture $f){}
