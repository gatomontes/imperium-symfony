...
use App\Imperium\Runtime\Citadel\Formation\{FormationJournal as J, FormationOwnerFrame as Owner, FormationPersonnel, FormationInstitution, FormationSignatures, FormationModelPreparation, FormationProfileDesignation};
...
if (isset($in['publication_phase'])) {
    ...
    $GLOBALS['ppc5_publication_barrier'] = static function (string $phase, string $to) use ($in, $root, $channel): void {
        ...
        file_put_contents($channel.'.held', $phase);
        if($in['publication_exception']??false) { throw new RuntimeException('CONTROLLED_PUBLICATION_EXCEPTION'); } $end = microtime(true) + 30;
...
try {
    if ($in['mode'] === 'publish') {
        ...
        $designation = new FormationProfileDesignation($j,$signatures,$clock,$institution,$personnel);
        $result = $designation->publish($in['envelope'],$in['candidate']);
    } elseif ($in['mode'] === 'current') {
        $personnel = new FormationPersonnel($j, $signatures, $clock, $institution);
        $designation = new FormationProfileDesignation($j,$signatures,$clock,$institution,$personnel);
        $result = $j->inspect(fn(array $frame, Owner $owner) => $designation->currentInOwner($owner,$in['seat']));
