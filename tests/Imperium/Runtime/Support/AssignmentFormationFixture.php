...
final class AssignmentFormationFixture
{
    public string $root;
    ...
    public function __construct(string $root, int $now, string $instance = 'instance-test')
    {
        $this->root = $root;
        // Existing parent institution is a distinct public-artifact input. Its
        // native producer performs no formation journal publication. O4 and all
        // formation authority, Profiles, sessions and custody use $root only.
        $institutionRoot=$root.'/synthetic-parent-institution';
        mkdir($institutionRoot,0700);
        ...
        $c->register(FormationInstitution::class)->setArguments([$institutionRoot])->setPublic(true);
        ...
        (new \App\Imperium\Runtime\Bootstrap\OperatorRootPersonnelInstallationService($institutionRoot))->install([
            'schema' => 'imperium.operator-root-personnel-package/v3', 'instance_id' => $instance, 'personnel' => $members]);
    ...
    {
        // The enclosing AssignmentFixture owns this generated root.
