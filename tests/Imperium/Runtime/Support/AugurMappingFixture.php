...
final class AugurMappingFixture
{
    public AugurFreshFixture $fresh;
    public AugurAdapter $adapter;
    public array $requests=[];
    public array $wires=[];
    public function __construct(array $limits, bool $firstOnly=false, public string $mode='valid', ?int $mappingLifetime=null)
    {
        $pins=[];
        $this->fresh=new AugurFreshFixture(configure:function(array &$p,OnboardingAuthorityFixture $f,array $profile)use(&$pins,$limits,$firstOnly,$mappingLifetime):void{
            // Configure the original map before actual policy signing and admission.
            // The synthetic factual verifiers and provider behavior remain unchanged.
            $oldMap=$p['body']['candidate_bindings'][0]['mapping_ref'];
            $map=Policy::content($f->sources[R::key($oldMap)],'runtime-binding-map');
            foreach($map['mappings'] as $index=>&$row){if(!$firstOnly || $index===0){$row['supported_limits']=$limits;}}unset($row);
            if($mappingLifetime!==null){$map['expires_at']=$f->now+$mappingLifetime;}
