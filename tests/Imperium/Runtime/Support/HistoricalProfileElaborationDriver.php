...
use App\Imperium\Runtime\Laboratorium\{ProfileElaborationCognitionGateway, ProfileDerivationCommissionAcceptanceService, ProfileCandidateDerivationService, ProfileCandidateReturnService};
...
final readonly class HistoricalProfileElaborationDriver
{
    ...
    public function run(string $root, string $senateDisposition = 'ACCEPTED'): array
    {
        ...
        HistoricalCuriaFixture::persist($store, ['proceeding_id' => 'proceeding-profile-elaboration-smoke', 'instance_id' => 'imperium-profile-elaboration-smoke']);
        HistoricalCuriaFixture::appendTurn($store, 'proceeding-profile-elaboration-smoke', 'response-profile-elaboration-smoke-plan', 1, [
        ...
        $seneschal = $this->seneschal();
        $this->write($root.'/var/imperium/offices/curia/occupancy/'.$seneschal['binding_id'].'.json', $seneschal);
    ...
    private function seneschal(): array
    {
        return $this->seal([
            'schema'=>'imperium.curia-seneschal-occupancy/v1','binding_id'=>'curia-seneschal-binding-'.str_repeat('9',20),
    ...
    private function lordSpeaker(): array
    {
        return $this->seal([
            ...
            'profile_examination_disposition_phase_opening_authority' => true,
        ]);
    }
