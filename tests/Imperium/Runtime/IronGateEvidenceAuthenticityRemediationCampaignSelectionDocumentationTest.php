...
final class IronGateEvidenceAuthenticityRemediationCampaignSelectionDocumentationTest extends TestCase
{
    public function testPreparationClassifiesAuthenticityGapsAndAuthorizesContractOnly(): void
    {
        ...
        self::assertStringContainsString('`BATCH_2_RESPONSE_ENVELOPE_PRODUCER_IMPLEMENTED`', $campaign);
    ...
    public function testPreparationKeepsLiveAndDeferredBoundariesClosed(): void
    {
        ...
        foreach (['runtime behavior', 'external I/O', '`AgentMailEmailSendCommand`', '`DeterministicBoundaryExecutor`', '`AgentMailEmailTransport`', '`IronGate`', 'expand Lazaretto', 'redesign credentials', 'assess sortie', 'revocation', 'telemetry', 'containment', 'incident'] as $boundary) self::assertStringContainsString($boundary, $campaign);
        self::assertStringContainsString('next separately selected campaign is Iron Gate Evidence Authenticity Remediation', $flow);
        self::assertStringContainsString('only envelope-bound raw-result consumption', $index);
