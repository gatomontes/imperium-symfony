...
final class InstalledRuntime
{
    ...
    public static function owner(bool $recovery = false): AuthorityOwner
    {
        ...
        $process=proc_open([WindowsPowerShell::EXECUTABLE,'-NoProfile','-NonInteractive','-File',
        ...
        if ($exit===2 && trim($out)==='PMA_RUNTIME_IDENTITY_REFUSED') throw new \RuntimeException('PMA_RUNTIME_IDENTITY_REFUSED');
        if ($exit!==0 || trim($out)!=='PMA_INSTALLATION_ACL_AND_IDENTITY_VERIFIED') throw new \RuntimeException('PMA_INSTALLATION_CHECK_FAILED');
