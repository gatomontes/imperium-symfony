...
final readonly class DelegateMissionUsabilityQuestionCommissionDispositionService
{
    ...
    public function decide(string $commissionId, string $usabilitySenatorBindingId, string $disposition, string $rationale, \DateTimeImmutable $decidedAt): array
    {
        if (!preg_match('/^delegate-mission-profile-examination-question-commission-[a-f0-9]{20}$/', $commissionId)) {
            throw new \InvalidArgumentException('S670_DELEGATE_MISSION_QUESTION_COMMISSION_ID_INVALID');
        }
        if (!preg_match('/^senate-committee-usability-binding-[a-f0-9]{20}$/', $usabilitySenatorBindingId)) {
            throw new \InvalidArgumentException('S671_DELEGATE_MISSION_USABILITY_SENATOR_BINDING_ID_INVALID');
        }
        $disposition = strtoupper(trim($disposition));
        $rationale = trim($rationale);
        if (!in_array($disposition, self::DISPOSITIONS, true) || '' === $rationale) {
            throw new \InvalidArgumentException('S672_DELEGATE_MISSION_QUESTION_COMMISSION_DISPOSITION_INVALID');
        }

        $commission = $this->read($this->commissions.'/'.$commissionId.'.json', 'S673_DELEGATE_MISSION_QUESTION_COMMISSION_ABSENT');
        $openingId = $commission['source_examination_opening']['id'] ?? '';
        $opening = $this->read($this->openings.'/'.$openingId.'.json', 'S674_DELEGATE_MISSION_EXAMINATION_OPENING_ABSENT');
        $custodyId = $commission['custody_lease']['custody_id'] ?? '';
        $custody = $this->read($this->custody.'/'.$custodyId.'.json', 'S675_DELEGATE_MISSION_EXAMINATION_CUSTODY_ABSENT');
        $usabilitySenator = $this->read($this->occupancy.'/'.$usabilitySenatorBindingId.'.json', 'S676_DELEGATE_MISSION_USABILITY_SENATOR_UNAVAILABLE');
        $this->validate($commissionId, $usabilitySenatorBindingId, $commission, $opening, $custody, $usabilitySenator);

        foreach (glob($this->dispositions.'/*.json') ?: [] as $path) {
            $prior = $this->read($path, 'S679_DELEGATE_MISSION_QUESTION_COMMISSION_DISPOSITION_CONFLICT');
            if (($prior['source_commission']['id'] ?? null) === $commissionId) {
                ...
                throw new \RuntimeException('S679_DELEGATE_MISSION_QUESTION_COMMISSION_DISPOSITION_CONFLICT');
            }
        }
        ...
        if ($accepted) {
            $authorshipAuthority = [
                'authority_id' => 'delegate-mission-usability-question-authorship-authority-'.substr(hash('sha256', CanonicalJson::encode([$dispositionId, $commission['hearing_contract']['subject'], 'usability', 1])), 0, 20),
                ...
                'purpose' => 'AUTHOR_ONE_BOUNDED_USABILITY_EXAMINATION_QUESTION',
        ...
        return $this->save($dispositionId, [
            ...
            'source_earlier_testimony_turn' => $commission['source_earlier_testimony_turn'],
            ...
            'status' => $accepted ? 'DELEGATE_MISSION_USABILITY_QUESTION_COMMISSION_ACCEPTED_PENDING_USABILITY_QUESTION_AUTHORSHIP' : 'DELEGATE_MISSION_USABILITY_QUESTION_COMMISSION_REFUSED_NO_QUESTION_AUTHORITY',
    ...
    private function validate(string $commissionId, string $usabilitySenatorBindingId, array $commission, array $opening, array $custody, array $usabilitySenator): void
    {
        $authority = $commission['recipient_acceptance_disposition_authority'] ?? null;
        if (!$this->valid($commission) || !$this->valid($opening) || !$this->valid($custody) || !$this->valid($usabilitySenator)
            ...
            || 'DELEGATE_MISSION_USABILITY_QUESTION_COMMISSION_ISSUED_PENDING_USABILITY_SENATOR_ACCEPTANCE' !== ($commission['status'] ?? null)
            || 'usability' !== ($commission['jurisdiction'] ?? null)
            ...
            || 'ACCEPT_OR_REFUSE_ONE_BOUNDED_USABILITY_QUESTION_COMMISSION' !== ($authority['purpose'] ?? null)
            || false !== ($authority['consumed'] ?? null)
            || ($commission['recipient']['binding_id'] ?? null) !== $usabilitySenatorBindingId
            || ($commission['recipient']['binding_digest'] ?? null) !== ($usabilitySenator['record_digest'] ?? null)
            || ($authority['holder']['binding_id'] ?? null) !== $usabilitySenatorBindingId
            || ($authority['holder']['binding_digest'] ?? null) !== ($usabilitySenator['record_digest'] ?? null)
            ...
            || 'imperium.senate-committee-occupancy/v1' !== ($usabilitySenator['schema'] ?? null)
            || 'senate.committee.usability' !== ($usabilitySenator['seat'] ?? null)
            || OfficerClass::Legate->value !== ($usabilitySenator['officer_class'] ?? null)
            || 'ACTIVE' !== ($usabilitySenator['status'] ?? null)
            || true !== ($usabilitySenator['binding_atomic'] ?? null)
            || true !== ($usabilitySenator['delegate_question_commission_acceptance_disposition_authority'] ?? null)
            || true === ($usabilitySenator['execution_authority'] ?? null)
            || ($commission['instance_id'] ?? null) !== ($usabilitySenator['instance_id'] ?? null)
            ...
            throw new \RuntimeException('S677_DELEGATE_MISSION_QUESTION_COMMISSION_DISPOSITION_CHAIN_INVALID');
        }
    }
    ...
    private function save(string $id, array $record): array
    {
        if (!is_dir($this->dispositions) && !mkdir($this->dispositions, 0770, true) && !is_dir($this->dispositions)) {
            throw new \RuntimeException('S678_DELEGATE_MISSION_QUESTION_COMMISSION_DISPOSITION_PERSISTENCE_FAILED');
        }
