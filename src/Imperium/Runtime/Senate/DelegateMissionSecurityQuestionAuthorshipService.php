...
final readonly class DelegateMissionSecurityQuestionAuthorshipService
{
    ...
    public function author(string $dispositionId, string $securitySenatorBindingId, \DateTimeImmutable $authoredAt): array
    {
        if (!preg_match('/^delegate-mission-profile-examination-question-commission-disposition-[a-f0-9]{20}$/', $dispositionId)) {
            throw new \InvalidArgumentException('S620_DELEGATE_MISSION_QUESTION_COMMISSION_DISPOSITION_ID_INVALID');
        }
        if (!preg_match('/^senate-committee-security-binding-[a-f0-9]{20}$/', $securitySenatorBindingId)) {
            throw new \InvalidArgumentException('S621_DELEGATE_MISSION_SECURITY_SENATOR_BINDING_ID_INVALID');
        }

        $disposition = $this->read($this->dispositions.'/'.$dispositionId.'.json', 'S622_DELEGATE_MISSION_QUESTION_COMMISSION_DISPOSITION_ABSENT');
        $commissionId = $disposition['source_commission']['id'] ?? '';
        $commission = $this->read($this->commissions.'/'.$commissionId.'.json', 'S623_DELEGATE_MISSION_QUESTION_COMMISSION_ABSENT');
        $openingId = $disposition['source_examination_opening']['id'] ?? '';
        $opening = $this->read($this->openings.'/'.$openingId.'.json', 'S624_DELEGATE_MISSION_EXAMINATION_OPENING_ABSENT');
        $custodyId = $disposition['custody_lease']['custody_id'] ?? '';
        $custody = $this->read($this->custody.'/'.$custodyId.'.json', 'S625_DELEGATE_MISSION_EXAMINATION_CUSTODY_ABSENT');
        $securitySenator = $this->read($this->occupancy.'/'.$securitySenatorBindingId.'.json', 'S626_DELEGATE_MISSION_SECURITY_SENATOR_UNAVAILABLE');
        $this->validate($dispositionId, $securitySenatorBindingId, $disposition, $commission, $opening, $custody, $securitySenator);

        foreach (glob($this->questions.'/*.json') ?: [] as $path) {
            $prior = $this->read($path, 'S629_DELEGATE_MISSION_SECURITY_QUESTION_CONFLICT');
            if (($prior['source_commission_disposition']['id'] ?? null) === $dispositionId) {
                ...
                throw new \RuntimeException('S629_DELEGATE_MISSION_SECURITY_QUESTION_CONFLICT');
            }
        }

        $authored = $this->cognition->authorQuestion('security', $commission, $opening);
        if (['purpose', 'question'] !== array_keys($authored)
            ...
            throw new \RuntimeException('S627_DELEGATE_MISSION_SECURITY_QUESTION_COGNITION_INVALID');
        }
        ...
        $dispatchAuthorization = [
            ...
            'purpose' => 'DECIDE_DISPATCH_OF_ONE_SEALED_SECURITY_EXAMINATION_QUESTION',
            'consumed' => false,
        ...
        return $this->save($questionId, [
            ...
            'status' => 'DELEGATE_MISSION_SECURITY_QUESTION_AUTHORED_SEALED_PENDING_DISPATCH_AUTHORIZATION',
    ...
    private function validate(string $dispositionId, string $bindingId, array $disposition, array $commission, array $opening, array $custody, array $senator): void
    {
        ...
        if (!$this->valid($disposition) || !$this->valid($commission) || !$this->valid($opening) || !$this->valid($custody) || !$this->valid($senator)
            ...
            || 'DELEGATE_MISSION_SECURITY_QUESTION_COMMISSION_ACCEPTED_PENDING_SECURITY_QUESTION_AUTHORSHIP' !== ($disposition['status'] ?? null)
            || !is_array($authority)
            || true !== ($authority['authority_single_use'] ?? null)
            || true !== ($authority['authority_exercisable'] ?? null)
            || 'AUTHOR_ONE_BOUNDED_SECURITY_EXAMINATION_QUESTION' !== ($authority['purpose'] ?? null)
            || 'security' !== ($authority['jurisdiction'] ?? null)
            ...
            || ($disposition['security_senator']['binding_id'] ?? null) !== $bindingId
            || ($disposition['security_senator']['binding_digest'] ?? null) !== ($senator['record_digest'] ?? null)
            ...
            || 'senate.committee.security' !== ($senator['seat'] ?? null)
            ...
            throw new \RuntimeException('S628_DELEGATE_MISSION_SECURITY_QUESTION_AUTHORSHIP_CHAIN_INVALID');
        }
    }
    ...
    {
        if (!is_dir($this->questions) && !mkdir($this->questions, 0770, true) && !is_dir($this->questions)) throw new \RuntimeException('S629_DELEGATE_MISSION_SECURITY_QUESTION_PERSISTENCE_FAILED');
        $record['record_digest'] = hash('sha256', CanonicalJson::encode($record));
        $path = $this->questions.'/'.$id.'.json';
        if (is_file($path)) {
            $prior = $this->read($path, 'S629_DELEGATE_MISSION_SECURITY_QUESTION_CONFLICT');
            if (CanonicalJson::encode($prior) !== CanonicalJson::encode($record)) throw new \RuntimeException('S629_DELEGATE_MISSION_SECURITY_QUESTION_CONFLICT');

            return $prior;
        }
        if (false === file_put_contents($path, json_encode($record, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_THROW_ON_ERROR)."\n", LOCK_EX)) throw new \RuntimeException('S629_DELEGATE_MISSION_SECURITY_QUESTION_PERSISTENCE_FAILED');
