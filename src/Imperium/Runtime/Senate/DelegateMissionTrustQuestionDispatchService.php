...
final readonly class DelegateMissionTrustQuestionDispatchService
{
    ...
    {
        if (!preg_match('/^delegate-mission-profile-examination-question-dispatch-decision-[a-f0-9]{20}$/', $decisionId)) throw new \InvalidArgumentException('S580_DELEGATE_MISSION_DISPATCH_DECISION_ID_INVALID');
        if (!preg_match('/^senate-bailiff-binding-[a-f0-9]{20}$/', $bailiffBindingId)) throw new \InvalidArgumentException('S581_DELEGATE_MISSION_BAILIFF_BINDING_ID_INVALID');
        $d = $this->read($this->decisions.'/'.$decisionId.'.json', 'S582_DELEGATE_MISSION_DISPATCH_DECISION_ABSENT');
        $questionId = $d['source_question']['id'] ?? '';
        $q = $this->read($this->questions.'/'.$questionId.'.json', 'S583_DELEGATE_MISSION_QUESTION_ABSENT');
        $custodyId = $d['custody_lease']['custody_id'] ?? '';
        $c = $this->read($this->custody.'/'.$custodyId.'.json', 'S584_DELEGATE_MISSION_EXAMINATION_CUSTODY_ABSENT');
        $b = $this->read($this->occupancy.'/'.$bailiffBindingId.'.json', 'S585_DELEGATE_MISSION_BAILIFF_UNAVAILABLE');
        $this->validate($decisionId, $bailiffBindingId, $d, $q, $c, $b);
        foreach (glob($this->dispatches.'/*.json') ?: [] as $p) {
            $x = $this->read($p, 'S589_DELEGATE_MISSION_QUESTION_DISPATCH_CONFLICT');
            if (($x['source_dispatch_decision']['id'] ?? null) === $decisionId) {
                ...
                throw new \RuntimeException('S589_DELEGATE_MISSION_QUESTION_DISPATCH_CONFLICT');
            }
        }
        ...
        $responseAuthority = [
            'authority_id' => 'delegate-mission-trust-testimony-response-authority-'.substr(hash('sha256', CanonicalJson::encode([$id, $d['manifestation']['manifestation_id'], $d['question']])), 0, 20),
            ...
            'purpose' => 'ANSWER_ONE_DISPATCHED_TRUST_EXAMINATION_QUESTION',
        ...
        return $this->save($id, [
            ...
            'trust_senator' => $d['trust_senator'], 'jurisdiction' => 'trust', 'question_sequence' => 1, 'question' => $d['question'],
            ...
            'status' => 'DELEGATE_MISSION_TRUST_QUESTION_DISPATCHED_UNCHANGED_PENDING_TESTIMONY_RESPONSE',
    ...
    private function validate(string $decisionId, string $bindingId, array $d, array $q, array $c, array $b): void
    {
        ...
        if (!$this->valid($d) || !$this->valid($q) || !$this->valid($c) || !$this->valid($b)
            ...
            || 'AUTHORIZED' !== ($d['disposition'] ?? null) || 'DELEGATE_MISSION_TRUST_QUESTION_DISPATCH_AUTHORIZED_PENDING_BAILIFF_DISPATCH' !== ($d['status'] ?? null)
            || !is_array($a) || true !== ($a['authority_single_use'] ?? null) || true !== ($a['authority_exercisable'] ?? null) || false !== ($a['consumed'] ?? null)
            || 'DISPATCH_ONE_SEALED_TRUST_EXAMINATION_QUESTION_UNCHANGED' !== ($a['purpose'] ?? null)
            ...
            throw new \RuntimeException('S586_DELEGATE_MISSION_QUESTION_DISPATCH_CHAIN_INVALID');
    }
    ...
    {
        if (!is_dir($this->dispatches) && !mkdir($this->dispatches, 0770, true) && !is_dir($this->dispatches)) throw new \RuntimeException('S587_DELEGATE_MISSION_QUESTION_DISPATCH_PERSISTENCE_FAILED');
        $r['record_digest'] = hash('sha256', CanonicalJson::encode($r)); $p = $this->dispatches.'/'.$id.'.json';
        if (is_file($p)) { $x = $this->read($p, 'S589_DELEGATE_MISSION_QUESTION_DISPATCH_CONFLICT'); if (CanonicalJson::encode($x) !== CanonicalJson::encode($r)) throw new \RuntimeException('S589_DELEGATE_MISSION_QUESTION_DISPATCH_CONFLICT'); return $x; }
        if (false === file_put_contents($p, json_encode($r, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_THROW_ON_ERROR)."\n", LOCK_EX)) throw new \RuntimeException('S587_DELEGATE_MISSION_QUESTION_DISPATCH_PERSISTENCE_FAILED');
        return $r;
    }
}
