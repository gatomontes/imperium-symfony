...
final readonly class SubordinatePersonaSecurityBaselineTestimonyService
{
    ...
    {
        if(!preg_match('/^senate-persona-baseline-question-[a-f0-9]{20}$/',$questionRecordId))throw new \InvalidArgumentException('S167_BASELINE_QUESTION_ID_INVALID');
        $q=$this->read('persona-questions',$questionRecordId,'S168_BASELINE_QUESTION_ABSENT');
        $d=$this->read('depositions',(string)($q['deposition_id']??''),'S169_SECURITY_TESTIMONY_CHAIN_INVALID');
        $w=$this->read('persona-witnesses',(string)($q['manifestation_id']??''),'S169_SECURITY_TESTIMONY_CHAIN_INVALID');
        $a=$q['testimony_authority']??null;
        if(!is_array($a)||'security'!==($q['jurisdiction']??null)||'SECURITY_BASELINE_QUESTION_SEALED_PENDING_TESTIMONY_COGNITION_AUTHORIZATION'!==($q['status']??null)
            ...
            ||!is_array($q['prior_testimony']??null)||3!==count($q['prior_testimony'])||null!==($q['testimony']??null)||true===($q['testimony_sealed']??null))
            throw new \RuntimeException('S169_SECURITY_TESTIMONY_CHAIN_INVALID');
        ...
        return $this->records->put('var/imperium/offices/senate/testimony-turns',$id,[
            ...
            'review_target_lineage'=>$q['review_target_lineage'],'source_first_testimony_turn'=>$q['source_first_testimony_turn'],'source_governance_testimony_turn'=>$q['source_governance_testimony_turn'],'source_consistency_testimony_turn'=>$q['source_consistency_testimony_turn'],'prior_testimony'=>$q['prior_testimony'],
    ...
    private function validate(array $answer):void{$keys=array_keys($answer);sort($keys,SORT_STRING);if(['answer','evidence_claims','refusals','uncertainties']!==$keys||!is_string($answer['answer'])||''===trim($answer['answer']))throw new \RuntimeException('S170_SECURITY_TESTIMONY_INVALID');foreach(['evidence_claims','refusals','uncertainties']as$field){if(!is_array($answer[$field])||!array_is_list($answer[$field]))throw new \RuntimeException('S170_SECURITY_TESTIMONY_INVALID');foreach($answer[$field]as$value)if(!is_string($value)||''===trim($value))throw new \RuntimeException('S170_SECURITY_TESTIMONY_INVALID');}}
}
