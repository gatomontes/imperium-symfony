...
final class ProfileSenateAuthorityTransition
{
    public static function run(string $resultDirectory, string $authorityId, \Closure $transition): array
    {
        ...
        return (new AtomicTransition($root))->run(
            'profile-senate-authority:'.hash('sha256', $authorityId),
            $transition,
        );
    }
    ...
    public static function seal(string $resultId, array $record, string $consumer): array
    {
        ...
        $scope = 'profile-senate-authority:'.hash('sha256', $authorityId);

        $record['transactional_consumption'] = TransactionalAuthorityConsumptionEnvelope::complete(
            'profile-senate-consumption-'.substr(hash('sha256', $authorityId.'|'.$resultId), 0, 20),
            ...
            [
                ...
                'bounded_act' => 'CONSUME_ONE_MODEL_BOUND_PROFILE_SENATE_OPENING_AUTHORITY',
            ],
    ...
    public static function isExactOrHistorical(array $record): bool
    {
        ...
        try {
            $expected = self::seal(
                self::resultId($record),
                $record,
                self::consumerFor($record),
            )['transactional_consumption'];
        } catch (\Throwable) {
            return false;
        }
    ...
    private static function consumption(array $record): array
    {
        ...
        throw new \InvalidArgumentException('S799_PROFILE_SENATE_TRANSACTION_INVALID');
    }
    ...
    {
        if (!is_array($record['lord_speaker'] ?? null)) {
            throw new \InvalidArgumentException('S799_PROFILE_SENATE_TRANSACTION_INVALID');
        }

        return ['role' => 'lord_speaker', 'identity' => $record['lord_speaker']];
    }

    private static function committedAt(array $record): \DateTimeImmutable
    {
        if (!is_string($record['opened_at'] ?? null)) {
            throw new \InvalidArgumentException('S799_PROFILE_SENATE_TRANSACTION_INVALID');
        }

        return new \DateTimeImmutable($record['opened_at']);
    }

    private static function resultId(array $record): string
    {
        if (!is_string($record['opening_id'] ?? null) || '' === trim($record['opening_id'])) {
            throw new \InvalidArgumentException('S799_PROFILE_SENATE_TRANSACTION_INVALID');
        }
    ...
    private static function consumerFor(array $record): string
    {
        return match ($record['schema'] ?? null) {
            'imperium.senate-model-bound-profile-examination-testimony-opening/v1' => ModelBoundProfileExaminationTestimonyOpeningService::class,
            'imperium.senate-model-bound-profile-finding-authority-opening/v1' => ModelBoundProfileFindingAuthorityOpeningService::class,
            'imperium.senate-model-bound-profile-deliberation-opening/v1' => ModelBoundProfileDeliberationOpeningService::class,
            default => throw new \InvalidArgumentException('S799_PROFILE_SENATE_TRANSACTION_INVALID'),
        };
    }
