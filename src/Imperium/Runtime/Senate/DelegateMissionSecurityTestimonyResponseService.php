...
final readonly class DelegateMissionSecurityTestimonyResponseService
{
    ...
    {
        if (!preg_match('/^delegate-mission-profile-examination-question-dispatch-[a-f0-9]{20}$/', $dispatchId)) throw new \InvalidArgumentException('S650_DELEGATE_MISSION_QUESTION_DISPATCH_ID_INVALID');
        $d = $this->read($this->dispatches.'/'.$dispatchId.'.json', 'S651_DELEGATE_MISSION_QUESTION_DISPATCH_ABSENT');
        $questionId = $d['source_question']['id'] ?? '';
        $q = $this->read($this->questions.'/'.$questionId.'.json', 'S652_DELEGATE_MISSION_QUESTION_ABSENT');
        $candidateId = $d['source_profile_candidate']['id'] ?? '';
        $candidate = $this->read($this->candidates.'/'.$candidateId.'.json', 'S653_DELEGATE_MISSION_PROFILE_CANDIDATE_ABSENT');
        $custodyId = $d['custody_lease']['custody_id'] ?? '';
        $custody = $this->read($this->custody.'/'.$custodyId.'.json', 'S654_DELEGATE_MISSION_EXAMINATION_CUSTODY_ABSENT');
        $this->validate($dispatchId, $d, $q, $candidate, $custody);
        foreach (glob($this->turns.'/*.json') ?: [] as $p) {
            $x = $this->read($p, 'S659_DELEGATE_MISSION_SECURITY_TESTIMONY_CONFLICT');
            if (($x['source_dispatch']['id'] ?? null) === $dispatchId) {
                if (($x['source_dispatch']['digest'] ?? null) === $d['record_digest']) return $x;
                throw new \RuntimeException('S659_DELEGATE_MISSION_SECURITY_TESTIMONY_CONFLICT');
            }
        }
        ...
        $nextAuthority = [
            'authority_id' => 'delegate-mission-usability-question-commission-authority-'.substr(hash('sha256', CanonicalJson::encode([$id, $d['hearing_contract']['subject'], 'usability'])), 0, 20),
            'authority_single_use' => true, 'authority_exercisable' => true, 'holder' => 'senate.lord-speaker',
            'purpose' => 'ISSUE_ONE_BOUNDED_USABILITY_QUESTION_COMMISSION', 'jurisdiction' => 'usability', 'question_limit' => 1,
        ...
        return $this->save($id, [
            ...
            'security_senator' => $d['security_senator'], 'jurisdiction' => 'security', 'question_sequence' => 2, 'question' => $d['question'], 'testimony' => $answer,
            ...
            'status' => 'DELEGATE_MISSION_SECURITY_TESTIMONY_RESPONSE_SEALED_PENDING_USABILITY_QUESTION_COMMISSION',
    ...
    private function validate(string $dispatchId, array $d, array $q, array $candidate, array $custody): void
    {
        ...
        if (!$this->valid($d) || !$this->valid($q) || !$this->valid($candidate) || !$this->valid($custody)
            ...
            || 'DELEGATE_MISSION_SECURITY_QUESTION_DISPATCHED_UNCHANGED_PENDING_TESTIMONY_RESPONSE' !== ($d['status'] ?? null)
            ...
            || 'ANSWER_ONE_DISPATCHED_SECURITY_EXAMINATION_QUESTION' !== ($a['purpose'] ?? null) || ($a['holder']['manifestation_id'] ?? null) !== ($d['manifestation']['manifestation_id'] ?? null)
            ...
            || true !== ($d['sealed'] ?? null)) throw new \RuntimeException('S655_DELEGATE_MISSION_SECURITY_TESTIMONY_CHAIN_INVALID');
    }
    private function validateAnswer(array $a): void
    {
        $keys = array_keys($a); sort($keys, SORT_STRING);
        if (['answer', 'evidence_claims', 'refusals', 'uncertainties'] !== $keys || !is_string($a['answer']) || '' === trim($a['answer'])) throw new \RuntimeException('S656_DELEGATE_MISSION_SECURITY_TESTIMONY_COGNITION_INVALID');
        foreach (['evidence_claims', 'refusals', 'uncertainties'] as $f) if (!is_array($a[$f]) || !array_is_list($a[$f]) || array_filter($a[$f], static fn($v): bool => !is_string($v) || '' === trim($v))) throw new \RuntimeException('S656_DELEGATE_MISSION_SECURITY_TESTIMONY_COGNITION_INVALID');
    }
    ...
    {
        if (!is_dir($this->turns) && !mkdir($this->turns, 0770, true) && !is_dir($this->turns)) throw new \RuntimeException('S658_DELEGATE_MISSION_SECURITY_TESTIMONY_PERSISTENCE_FAILED');
        $r['record_digest'] = hash('sha256', CanonicalJson::encode($r)); $p = $this->turns.'/'.$id.'.json';
        if (is_file($p)) { $x = $this->read($p, 'S659_DELEGATE_MISSION_SECURITY_TESTIMONY_CONFLICT'); if (CanonicalJson::encode($x) !== CanonicalJson::encode($r)) throw new \RuntimeException('S659_DELEGATE_MISSION_SECURITY_TESTIMONY_CONFLICT'); return $x; }
        if (false === file_put_contents($p, json_encode($r, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_THROW_ON_ERROR)."\n", LOCK_EX)) throw new \RuntimeException('S658_DELEGATE_MISSION_SECURITY_TESTIMONY_PERSISTENCE_FAILED');
        return $r;
    }
}
