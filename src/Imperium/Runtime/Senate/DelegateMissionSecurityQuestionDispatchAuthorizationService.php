...
final readonly class DelegateMissionSecurityQuestionDispatchAuthorizationService
{
    ...
    {
        if (!preg_match('/^delegate-mission-profile-examination-question-[a-f0-9]{20}$/', $questionId)) throw new \InvalidArgumentException('S630_DELEGATE_MISSION_QUESTION_ID_INVALID');
        if (!preg_match('/^senate-lord-speaker-binding-[a-f0-9]{20}$/', $lordSpeakerBindingId)) throw new \InvalidArgumentException('S631_DELEGATE_MISSION_LORD_SPEAKER_BINDING_ID_INVALID');
        $disposition = strtoupper(trim($disposition));
        $rationale = trim($rationale);
        if (!in_array($disposition, self::DISPOSITIONS, true) || '' === $rationale) throw new \InvalidArgumentException('S632_DELEGATE_MISSION_QUESTION_DISPATCH_DISPOSITION_INVALID');

        $question = $this->read($this->questions.'/'.$questionId.'.json', 'S633_DELEGATE_MISSION_QUESTION_ABSENT');
        $admissionId = $question['source_stand_admission']['id'] ?? '';
        $admission = $this->read($this->admissions.'/'.$admissionId.'.json', 'S634_DELEGATE_MISSION_STAND_ADMISSION_ABSENT');
        $lordSpeaker = $this->read($this->occupancy.'/'.$lordSpeakerBindingId.'.json', 'S635_DELEGATE_MISSION_LORD_SPEAKER_UNAVAILABLE');
        $bailiffId = $admission['bailiff']['binding_id'] ?? '';
        $bailiff = $this->read($this->occupancy.'/'.$bailiffId.'.json', 'S636_DELEGATE_MISSION_BAILIFF_UNAVAILABLE');
        ...
        foreach (glob($this->decisions.'/*.json') ?: [] as $path) {
            $prior = $this->read($path, 'S639_DELEGATE_MISSION_QUESTION_DISPATCH_DECISION_CONFLICT');
            if (($prior['source_question']['id'] ?? null) === $questionId) {
                ...
                throw new \RuntimeException('S639_DELEGATE_MISSION_QUESTION_DISPATCH_DECISION_CONFLICT');
            }
        }
        ...
        $dispatchAuthority = $authorized ? [
            ...
            'purpose' => 'DISPATCH_ONE_SEALED_SECURITY_EXAMINATION_QUESTION_UNCHANGED',
        ...
        return $this->save($id, [
            ...
            'security_senator' => $question['security_senator'], 'jurisdiction' => 'security', 'question_sequence' => 2, 'question' => $question['question'],
            ...
            'status' => $authorized ? 'DELEGATE_MISSION_SECURITY_QUESTION_DISPATCH_AUTHORIZED_PENDING_BAILIFF_DISPATCH' : 'DELEGATE_MISSION_SECURITY_QUESTION_DISPATCH_REFUSED_NO_TESTIMONY_AUTHORITY',
    ...
    private function validate(string $questionId, string $bindingId, array $q, array $a, array $l, array $b): void
    {
        ...
        if (!$this->valid($q) || !$this->valid($a) || !$this->valid($l) || !$this->valid($b)
            ...
            || 'DELEGATE_MISSION_SECURITY_QUESTION_AUTHORED_SEALED_PENDING_DISPATCH_AUTHORIZATION' !== ($q['status'] ?? null)
            ...
            || 'senate.lord-speaker' !== ($authority['holder'] ?? null) || 'DECIDE_DISPATCH_OF_ONE_SEALED_SECURITY_EXAMINATION_QUESTION' !== ($authority['purpose'] ?? null) || false !== ($authority['consumed'] ?? null)
            ...
            throw new \RuntimeException('S637_DELEGATE_MISSION_QUESTION_DISPATCH_DECISION_CHAIN_INVALID');
    }
    ...
    {
        if (!is_dir($this->decisions) && !mkdir($this->decisions, 0770, true) && !is_dir($this->decisions)) throw new \RuntimeException('S638_DELEGATE_MISSION_QUESTION_DISPATCH_DECISION_PERSISTENCE_FAILED');
        $r['record_digest'] = hash('sha256', CanonicalJson::encode($r)); $p = $this->decisions.'/'.$id.'.json';
        if (is_file($p)) { $x = $this->read($p, 'S639_DELEGATE_MISSION_QUESTION_DISPATCH_DECISION_CONFLICT'); if (CanonicalJson::encode($x) !== CanonicalJson::encode($r)) throw new \RuntimeException('S639_DELEGATE_MISSION_QUESTION_DISPATCH_DECISION_CONFLICT'); return $x; }
        if (false === file_put_contents($p, json_encode($r, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_THROW_ON_ERROR)."\n", LOCK_EX)) throw new \RuntimeException('S638_DELEGATE_MISSION_QUESTION_DISPATCH_DECISION_PERSISTENCE_FAILED');
        return $r;
    }
}
