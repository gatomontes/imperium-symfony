...
final readonly class SymfonyAiProfileExaminationQuestionCognitionGateway implements ProfileExaminationQuestionCognitionGateway
{
    ...
    {
        if (!in_array($jurisdiction, ['trust','security','usability'], true)) throw new \RuntimeException('S220_PROFILE_EXAMINATION_QUESTION_COGNITION_INVALID');
        $prompt = implode("\n", [
        ...
        $content = $this->cognition->invoke('senate-profile-examination','question-'.$jurisdiction,(string)($commission['commission_id']??''),'senate.committee.'.$jurisdiction,'author-profile-question',[$commission,$opening],$prompt);
