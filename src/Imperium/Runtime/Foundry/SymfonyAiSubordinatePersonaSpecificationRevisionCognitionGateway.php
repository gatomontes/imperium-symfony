...
final readonly class SymfonyAiSubordinatePersonaSpecificationRevisionCognitionGateway implements SubordinatePersonaSpecificationRevisionCognitionGateway
{
    ...
    {
        $prompt = implode("\n", [
            'Exact construction case: '.json_encode($case, JSON_UNESCAPED_SLASHES | JSON_THROW_ON_ERROR),
            'Exact prior Persona specification: '.json_encode($priorSpecification, JSON_UNESCAPED_SLASHES | JSON_THROW_ON_ERROR),
            'Exact returned clarification: '.json_encode($clarificationReturn, JSON_UNESCAPED_SLASHES | JSON_THROW_ON_ERROR),
            'Revise only the bounded Persona specification. Preserve the returned clarification verbatim in lineage; do not answer it by inventing facts.',
        ...
        if (!is_string($content) || '' === trim($content)) {
            throw new \RuntimeException('F128_PERSONA_SPECIFICATION_REVISION_COGNITION_EMPTY');
        }
        ...
        try {
            ...
            throw new \RuntimeException('F129_PERSONA_SPECIFICATION_REVISION_COGNITION_INVALID', 0, $exception);
        }
        $expected = ['behavioral_directives', 'competencies', 'disposition', 'evidence_obligations', 'explicit_exclusions', 'identity_constraints', 'persona_name', 'purpose', 'return_contracts', 'source_requirements', 'stop_conditions'];
        $keys = is_array($decision) ? array_keys($decision) : [];
        sort($keys, SORT_STRING);
        if ($expected !== $keys
            || !in_array($decision['disposition'] ?? null, ['PERSONA_SPECIFICATION_COMPLETE', 'CLARIFICATION_REQUIRED'], true)
            || !is_string($decision['persona_name'] ?? null)
            || '' === trim($decision['persona_name'])
            || !is_string($decision['purpose'] ?? null)
            || '' === trim($decision['purpose'])
        ) {
            throw new \RuntimeException('F130_PERSONA_SPECIFICATION_REVISION_CONTRACT_INVALID');
        }
        foreach (array_diff($expected, ['disposition', 'persona_name', 'purpose']) as $field) {
