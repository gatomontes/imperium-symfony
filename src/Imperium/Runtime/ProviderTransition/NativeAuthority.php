...
final readonly class NativeAuthority
{
    ...
    public function issue(string $principalId): array
    {
        return $this->state->locked(function () use ($principalId): array {
            $at = null === $this->clock ? time() : ($this->clock)();
            $p = (new NativePrincipal($this->state))->load($principalId, $at);
            $chain = $this->chain($p, $at);
    ...
    public function load(string $id, int $at): array
    {
        ...
        if ($r !== $this->chain($p, $r['at']) || $id !== $r['authority']['authority_id']) { throw new \RuntimeException('NIR_AUTHORITY_LINEAGE'); }
        return $r;
    }

    private function chain(array $p, int $at): array
    {
        $id = 'native-authority-'.TransitionContract::digest([$p['instance_id'], $p['principal_version_id'], $p['operation'], $p['provider_binding']]);
        ...
        $decision = NativeState::seal(['schema' => 'imperium.imperator.native-transition-decision/v1', 'decision_id' => 'decision-'.$root,
            ...
            'operation' => $p['operation'], 'issuance_target' => $target, 'at' => $at, 'expires_at' => $p['expires_at'],
