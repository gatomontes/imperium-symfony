...
final readonly class Recovery
{
    ...
    public function resume(string $raw):array {
        R::require(strlen($raw)<=1048576,'REQUEST_LIMIT');$q=StrictJson::decode($raw);R::object($q,['schema','sequence_id','command_id','expected_head','recognize_command_ref']);R::require($q['schema']==='imperium.provider-onboarding-resume/v2','REQUEST_SCHEMA');R::id($q['sequence_id']);R::id($q['command_id']);CommandLedger::head($q['expected_head']);
        ...
        $out=$this->ledger->store->journal->changeAtHead(function(array &$state,array $head)use($q,$raw):array{
            $store=$this->ledger->store;$s=$store->state($state);$key=LedgerState::key('command',[$store->instance,$q['sequence_id'],$q['command_id']]);
            if(isset($s['commands'][$key])){R::require(R::same($s['commands'][$key]['request'],$q),'COMMAND_CONFLICT');return ['result'=>CommandLedger::presentation($s['commands'][$key],$head,true),'claims'=>[]];}
            R::require(R::same(CommandLedger::head($q['expected_head']),$head),'STALE_HEAD');$original=LedgerState::command($s,$q['recognize_command_ref']);R::require($original['ref']['sequence_id']===$q['sequence_id'],'RESUME_SOURCE');R::require(count($s['commands'])<4096,'LIMIT_EXCEEDED');
            ...
            return ['result'=>CommandLedger::presentation($c,$head,false),'claims'=>$ids];
        });
