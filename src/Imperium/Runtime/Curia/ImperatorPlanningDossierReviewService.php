...
final readonly class ImperatorPlanningDossierReviewService
{
    ...
    public function __construct(private string$root){$this->dossiers=$root.'/var/imperium/offices/curia/planning-dossiers';$this->reviews=$root.'/var/imperium/offices/curia/planning-dossier-reviews';}
    ...
    {
        $existing=$persist?$this->existing($authorityId):null;if(null!==$existing)return$existing;$d=$this->read($this->dossiers.'/'.$dossierId.'.json','C250_PLANNING_DOSSIER_ABSENT');(new \App\Imperium\Runtime\Citadel\Formation\LegacyFormationGuard($this->root))->requireDossier($d);$a=$d['imperator_review_authority']??[];$numbers=$this->numbers($citedLineNumbers);$approval='APPROVE_DOSSIER'===$disposition;
