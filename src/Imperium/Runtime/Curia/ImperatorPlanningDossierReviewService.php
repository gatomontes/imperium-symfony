...
final readonly class ImperatorPlanningDossierReviewService
{
    ...
    public function review(string$dossierId,string$authorityId,string$disposition,array$citedLineNumbers,string$response,bool$acknowledgeAllLines,\DateTimeImmutable$reviewedAt):array
    {
        $existing=$this->existing($authorityId);if(null!==$existing)return$existing;$d=$this->read($this->dossiers.'/'.$dossierId.'.json','C250_PLANNING_DOSSIER_ABSENT');$a=$d['imperator_review_authority']??[];$numbers=$this->numbers($citedLineNumbers);$approval='APPROVE_DOSSIER'===$disposition;
