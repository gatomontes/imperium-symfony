...
final readonly class OperationalAdoptionReconciliationService
{
    ...
    public function reconcile(string $openingId, string $authorityId, string $seneschalBindingId, array $authoredReconciliation, \DateTimeImmutable $reconciledAt): array
    {
        ...
        return OperationalAdoptionAuthorityTransition::run(
            ...
            $authorityId,
            fn (): array => $this->reconcileLocked($openingId, $authorityId, $seneschalBindingId, $authoredReconciliation, $reconciledAt),
        );
    }

    private function reconcileLocked(string $openingId, string $authorityId, string $seneschalBindingId, array $authoredReconciliation, \DateTimeImmutable $reconciledAt): array
    {
        ...
        foreach (glob($this->reconciliations.'/*.json') ?: [] as $path) {
            ...
            if (($prior['source_reconciliation_opening']['id'] ?? null) === $openingId) {
                if (!$this->valid($prior) || ($prior['source_reconciliation_opening']['digest'] ?? null) !== ($opening['record_digest'] ?? null) || ($prior['reconciler']['binding_id'] ?? null) !== $seneschalBindingId || ($prior['reconciliation'] ?? null) !== $reconciliation || !OperationalAdoptionAuthorityTransition::isExactOrHistorical($prior)) throw new \RuntimeException('CUR523_RECONCILIATION_CONFLICT');
                return $prior;
            }
        }
        ...
        return OperationalAdoptionAuthorityTransition::put($this->reconciliations, $id, [
        ...
        ], self::class, 'CUR525_RECONCILIATION_PERSISTENCE_FAILED', 'CUR523_RECONCILIATION_CONFLICT');
    }
