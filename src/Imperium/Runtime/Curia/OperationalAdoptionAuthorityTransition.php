...
final class OperationalAdoptionAuthorityTransition
{
    public static function run(string $resultDirectory, string $authorityId, \Closure $transition): array
    {
        $root = dirname($resultDirectory, 4);

        return (new AtomicTransition($root))->run(
            'operational-adoption-authority:'.hash('sha256', $authorityId),
            $transition,
        );
    }
    ...
    public static function seal(string $resultId, array $record, string $consumer): array
    {
        ...
        $scope = 'operational-adoption-authority:'.hash('sha256', $authorityId);

        $record['transactional_consumption'] = TransactionalAuthorityConsumptionEnvelope::complete(
            'operational-adoption-consumption-'.substr(hash('sha256', $authorityId.'|'.$resultId), 0, 20),
            ...
            [
                ...
                'bounded_act' => 'CONSUME_ONE_OPERATIONAL_ADOPTION_AUTHORITY',
            ],
    ...
    private static function consumption(array $record): array
    {
        ...
        throw new \InvalidArgumentException('CUR599_OPERATIONAL_ADOPTION_TRANSACTION_INVALID');
    }
    ...
    {
        foreach (['reconciler', 'decision_maker'] as $key) {
    ...
    {
        foreach (['reconciled_at', 'decided_at'] as $key) {
    ...
    {
        foreach (['reconciliation_id', 'disposition_id'] as $key) {
    ...
    private static function consumerFor(array $record): string
    {
        return match ($record['schema'] ?? null) {
            'imperium.legate-result-adoption-reconciliation/v1' => OperationalAdoptionReconciliationService::class,
            'imperium.legate-result-adoption-disposition/v1' => OperationalAdoptionDispositionService::class,
            default => throw new \InvalidArgumentException('CUR599_OPERATIONAL_ADOPTION_TRANSACTION_INVALID'),
        };
    }
