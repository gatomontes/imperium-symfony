...
final readonly class GovernedStationaryCredentialResolutionService
{
    public const string PROOFS = 'var/imperium/offices/clavium/stationary-credential-resolution-proofs';
    ...
    {
        if (!preg_match(
            '/^governed-provider-execution-admission-[a-f0-9]{20}$/',
            $admissionId,
        )) {
        ...
        $admission = $this->references->read(
            $this->root.'/'.GovernedProviderExecutionAdmissionService::ADMISSIONS
                .'/'.$admissionId.'.json',
        ...
        return $this->atomic->run(
            'stationary-credential-resolution:'.$admissionId,
    ...
    private function resolveAndValidate(
        ...
        return [$authority, $principal, $binding];
    }
