...
final readonly class GarrisonInventoryInquiryService
{
    ...
    public function route(string $determinationId): array
    {
        if (
            !preg_match(
                '/^guildhall-determination-[a-f0-9]{20}$/',
                $determinationId,
            )
        ) {
            throw new \InvalidArgumentException(
                "GA10_DETERMINATION_INVALID: exact Guildhall determination identity is required.",
            );
        }
        $determination = $this->read(
            $this->determinationDirectory . "/" . $determinationId . ".json",
            "GA11_DETERMINATION_ABSENT",
        );
        $synthesis = $determination["guildmaster_synthesis"] ?? null;
        if (
            !is_array($synthesis) ||
            !$this->digestMatches($determination) ||
            "imperium.guildhall-profession-determination/v1" !==
                ($determination["schema"] ?? null) ||
            "PROFESSION_DETERMINED_GARRISON_INVENTORY_REQUIRED" !==
                ($determination["status"] ?? null) ||
            "PROFESSION_DETERMINATION_COMPLETE" !==
                ($synthesis["disposition"] ?? null) ||
            ...
            throw new \RuntimeException(
                "GA12_DETERMINATION_INVALID: exact sealed non-executing Profession Determination is required.",
            );
        }
        ...
        $inquiry = [
            ...
            "requester" => [
                ...
                "manifestation_id" =>
                    $determination["occupancy"]["guildhall.guildmaster"][
                        "manifestation_id"
                    ] ?? null,
            ...
            "requested_facts" => [
                ...
                "custody state",
                "availability facts",
                "qualification evidence held in custody",
    ...
    private function currentConstable(): ?array
    {
        ...
        if (1 !== count($paths)) {
            throw new \RuntimeException(
                "GA13_CONSTABLE_OCCUPANCY_AMBIGUOUS: exact current Constable occupancy cannot be established.",
            );
        }
        ...
        if (
            ...
            throw new \RuntimeException(
                "GA14_CONSTABLE_OCCUPANCY_INVALID: exact active Constable occupancy is required.",
            );
        }
    ...
    {
        if (
            !is_dir($this->inquiryDirectory) &&
            !mkdir($this->inquiryDirectory, 0770, true) &&
            !is_dir($this->inquiryDirectory)
        ) {
            throw new \RuntimeException("Garrison inbox cannot be created.");
        }
        $inquiry["record_digest"] = hash(
            "sha256",
            CanonicalJson::encode($inquiry),
        );
        $path =
            $this->inquiryDirectory . "/" . $inquiry["inquiry_id"] . ".json";
        if (is_file($path)) {
            ...
            if (
                ...
                throw new \RuntimeException(
                    "GA15_INQUIRY_REPLAY_CONFLICT: inquiry identity is already bound differently.",
                );
            }
            return $existing;
        }
        ...
        if (
            false === file_put_contents($temporary, $json, LOCK_EX) ||
            !rename($temporary, $path)
        ) {
            @unlink($temporary);
            throw new \RuntimeException(
                "Garrison inquiry cannot be committed atomically.",
            );
        }
        return $inquiry;
    }
