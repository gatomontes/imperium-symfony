...
final readonly class GarrisonInventoryResponseService
{
    ...
    public function __construct(string $projectDir)
    {
        ...
        $this->custodyDirectory =
            $projectDir . "/var/imperium/offices/garrison/custody";
        $this->guildhallResponses =
            $projectDir . "/var/imperium/offices/guildhall/inventory-responses";
    }
    ...
    public function respond(string $inquiryId): array
    {
        if (!preg_match('/^garrison-inquiry-[a-f0-9]{20}$/', $inquiryId)) {
            throw new \InvalidArgumentException(
                "GA60_INQUIRY_INVALID: exact Garrison inquiry identity is required.",
            );
        }
        $inquiry = $this->read(
            $this->inquiryDirectory . "/" . $inquiryId . ".json",
            "GA61_INQUIRY_ABSENT",
        );
        if (
            !$this->digestMatches($inquiry) ||
            ...
            true === ($inquiry["ranking_authority"] ?? null) ||
            true === ($inquiry["selection_authority"] ?? null) ||
            true === ($inquiry["reservation_authority"] ?? null) ||
            true === ($inquiry["retrieval_authority"] ?? null) ||
            true === ($inquiry["spawning_authority"] ?? null) ||
            true === ($inquiry["execution_authority"] ?? null)
        ) {
            throw new \RuntimeException(
                "GA62_INQUIRY_INVALID: exact pending non-authorizing Garrison inquiry is required.",
            );
        }
        ...
        $response = [
            ...
            "proceeding_id" => $inquiry["proceeding_id"],
            ...
            "inventory_questions" => $inquiry["inventory_questions"],
            "requested_facts" => $inquiry["requested_facts"],
            "inventory_records" => $records,
            "ledger_finding" =>
                [] === $records
                    ? "NO_ADMITTED_PERSONA_CUSTODY_RECORDS_HELD"
                    : "EXACT_ADMITTED_PERSONA_CUSTODY_RECORDS_ATTACHED",
            ...
            "interpretation_boundary" =>
                "Garrison reports exact custody and availability facts only; Guildhall determines professional fit and construction need.",
        ];
    ...
    private function currentConstable(mixed $instanceId): array
    {
        ...
        if (1 !== count($paths)) {
            throw new \RuntimeException(
                "GA63_CONSTABLE_OCCUPANCY_REQUIRED: one exact active Constable occupancy is required.",
            );
        }
        ...
        if (
            ...
            throw new \RuntimeException(
                "GA64_CONSTABLE_OCCUPANCY_INVALID: active occupancy lacks exact inventory-response authority.",
            );
        }
        return $record;
    }
    ...
    private function custodyRecords(): array
    {
        ...
        foreach (glob($this->custodyDirectory . "/*.json") ?: [] as $path) {
            ...
            if (
                ...
                throw new \RuntimeException(
                    "GA65_CUSTODY_RECORD_INVALID: inventory contains an invalid custody record.",
                );
            }
            $records[] = $record;
        }
        usort(
            ...
            static fn(array $a, array $b): int => [
                $a["persona_id"],
                $a["persona_version"],
            ] <=> [$b["persona_id"], $b["persona_version"]],
        );
        return $records;
    }
    ...
    {
        if (
            !is_dir($this->guildhallResponses) &&
            !mkdir($this->guildhallResponses, 0770, true) &&
            !is_dir($this->guildhallResponses)
        ) {
            throw new \RuntimeException(
                "Guildhall inventory-response directory cannot be created.",
            );
        }
        $response["record_digest"] = hash(
            "sha256",
            CanonicalJson::encode($response),
        );
        $path =
            $this->guildhallResponses .
            "/" .
            ...
            ".json";
        if (is_file($path)) {
            $existing = $this->read($path, "GA66_RESPONSE_ABSENT");
            if (
                CanonicalJson::encode($existing) !==
                CanonicalJson::encode($response)
            ) {
                throw new \RuntimeException("GA67_RESPONSE_REPLAY_CONFLICT");
            }
            return $existing;
        }
        ...
        if (
            ...
            @unlink($temporary);
            throw new \RuntimeException(
                "Garrison inventory response cannot be committed atomically.",
            );
        }
        return $response;
    }
