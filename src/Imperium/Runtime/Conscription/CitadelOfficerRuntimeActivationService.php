...
final readonly class CitadelOfficerRuntimeActivationService
{
    ...
    {
        $this->authorizations = $root.'/var/imperium/imperator/citadel-officer-activation-authorization-decisions';
    ...
    {
        if (!preg_match('/^citadel-officer-activation-authorization-decision-[a-f0-9]{20}$/', $decisionId)) {
        ...
        $activationId = 'citadel-officer-runtime-activation-'.substr(hash('sha256', CanonicalJson::encode([$decisionId, $decision['record_digest'], $binding['record_digest'], $actor, $activatedAt->format(DATE_ATOM)])), 0, 20);
    ...
    private function validate(string $decisionId, array $decision, array $binding, array $assembly, array $qualification, array $approval, array $modelBinding, array $attestation, string $instanceId, \DateTimeImmutable $activatedAt): void
    {
        ...
        if (!$this->valid($decision) || 'imperium.imperator-citadel-officer-activation-authorization-decision/v1' !== ($decision['schema'] ?? null)
            ...
            || 'MODEL_BOUND_MANIFESTATION_ACTIVATION_AUTHORIZED_PENDING_RUNTIME_ACTIVATION' !== ($decision['status'] ?? null)
            || true !== ($authority['authority_single_use'] ?? null) || false !== ($authority['consumed'] ?? null)
            || 'conscription.recruiter' !== ($authority['destination'] ?? null) || 'ACTIVATE_EXACT_BOUND_CITADEL_MANIFESTATION' !== ($authority['purpose'] ?? null)
