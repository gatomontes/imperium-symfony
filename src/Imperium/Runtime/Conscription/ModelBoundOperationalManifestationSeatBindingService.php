...
final readonly class ModelBoundOperationalManifestationSeatBindingService
{
    ...
    public function bind(string $id): array
    {
        ...
        'R221_MODEL_BOUND_ASSEMBLY_ABSENT');
        foreach (glob($this->o.'/*.json') ? :[] as $p) {
            $x = $this->read($p,
            'R225_MODEL_BOUND_BINDING_CONFLICT');
            if (($x['source_assembly']['id'] ?? null) === $id) {
                if (($x['source_assembly']['digest'] ?? null) !== ($a['record_digest'] ?? null)) throw new \RuntimeException('R225_MODEL_BOUND_BINDING_CONFLICT');
                return $x;
            }
            if (($x['seat'] ?? null) === ($a['manifestation']['intended_seat']['id'] ?? null) &&
            in_array($x['status'] ?? null,
            ['OPERATIONAL_MANIFESTATION_BOUND_PENDING_DEPLOYMENT_AUTHORIZATION',
            'ACTIVE'],
            true)) throw new \RuntimeException('R224_MODEL_BOUND_TARGET_SEAT_OCCUPIED');
        }
        $q = $this->read($this->q.'/'.($a['source_qualification']['id'] ?? '').'.json',
        'R222_MODEL_BOUND_QUALIFICATION_ABSENT');
        [$instance,
        $r] = $this->recruiter();
        $m = $a['manifestation'] ?? [];
        $auth = $a['seat_binding_authority'] ?? [];
        if (!$this->ok($a) ||
        !$this->ok($q) ||
        'imperium.conscription-model-bound-operational-manifestation-assembly/v1' !== ($a['schema'] ?? null) ||
        $instance !== ($a['instance_id'] ?? null) ||
        'OPERATIONAL_MANIFESTATION_ASSEMBLED_PENDING_SEAT_BINDING' !== ($a['status'] ?? null) ||
        true !== ($a['operational_manifestation_assembled'] ?? null) ||
        true !== ($auth['authority_single_use'] ?? null) ||
        false !== ($auth['consumed'] ?? null) ||
        true !== ($a['seat_binding_authority_exercisable'] ?? null) ||
        false !== ($a['seat_bound'] ?? null) ||
        'ASSEMBLED_UNBOUND' !== ($m['status'] ?? null) ||
        false !== ($m['operational_use_permitted'] ?? null) ||
        true === ($a['deployment_authority'] ?? null) ||
        true === ($a['custody_transfer_authority'] ?? null) ||
        true === ($a['tool_use_authority'] ?? null) ||
        true === ($a['credential_use_authority'] ?? null) ||
        true === ($a['external_action_authority'] ?? null) ||
        true === ($a['execution_authority'] ?? null) ||
        ($a['source_qualification']['digest'] ?? null) !== $q['record_digest'] ||
        ($m['profile'] ?? null) !== $q['operational_profile'] ||
        ($m['intended_seat'] ?? null) !== $q['intended_seat']) throw new \RuntimeException('R223_MODEL_BOUND_BINDING_CHAIN_INVALID');
        $seat = $m['intended_seat']['id'];
        $actor = ['seat' => 'conscription.recruiter',
        'manifestation_id' => $r['manifestation_id'],
        'occupancy_generation' => $r['occupancy_generation']];
        $bid = 'model-bound-operational-seat-binding-'.substr(hash('sha256',
        ...
        'binding_atomic' => true,
        'seat_binding_authority' => ['id' => $auth['authority_id'],
        'consumed' => true,
    ...
    private function recruiter(): array
    {
        $s = $this->b->read();
        if (!is_array($s) ||
        BootstrapState::CuriaReady->value !== ($s['state'] ?? null)) throw new \RuntimeException('R226_MODEL_BOUND_RECRUITER_UNAVAILABLE');
        ...
        throw new \RuntimeException('R226_MODEL_BOUND_RECRUITER_UNAVAILABLE');
    }
    ...
    array $r): array {
        if (!is_dir($this->o))mkdir($this->o,
        0770,
        true);
        $r['record_digest'] = hash('sha256',
        CanonicalJson::encode($r));
        file_put_contents($this->o.'/'.$id.'.json',
        json_encode($r,
