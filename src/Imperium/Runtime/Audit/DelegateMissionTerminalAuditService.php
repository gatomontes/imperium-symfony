...
final readonly class DelegateMissionTerminalAuditService
{
    ...
    public function audit(string $terminalId): array
    {
        ...
        $return = $this->source('var/imperium/offices/curia/delegate-mission-return-authorizations', $terminal['source_return_authorization'] ?? [], 'AUD302_DELEGATE_RETURN_AUTHORIZATION_INVALID');
        $disposition = $this->source('var/imperium/offices/curia/delegate-mission-cognition-result-dispositions', $return['source_disposition'] ?? [], 'AUD303_DELEGATE_RESULT_DISPOSITION_INVALID');
        $turn = $this->source('var/imperium/operational/delegate-mission-bounded-cognition-turns', $return['source_turn'] ?? [], 'AUD304_DELEGATE_COGNITION_TURN_INVALID');
        $activation = $this->source('var/imperium/offices/clavium/delegate-mission-provider-invocation-activations', $turn['source_activation'] ?? [], 'AUD305_DELEGATE_PROVIDER_ACTIVATION_INVALID');
        $resourceDecision = $this->source('var/imperium/imperator/delegate-mission-resource-invocation-decisions', $activation['source_authorization'] ?? [], 'AUD306_DELEGATE_RESOURCE_DECISION_INVALID');
        ...
        return [
            'status' => 'DELEGATE_MISSION_TERMINAL_CHAIN_VALID',
    ...
    private function source(string $directory, array $reference, string $error): array
    {
        ...
        if (($reference['digest'] ?? null) !== $record['record_digest']) {
