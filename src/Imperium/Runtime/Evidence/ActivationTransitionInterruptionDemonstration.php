...
final readonly class ActivationTransitionInterruptionDemonstration
{
    ...
    private function fixture(string $root, string $transition, \DateTimeImmutable $at): array
    {
        ...
        $principalVersionId = 'imperator-offline-demonstration-version-1';
        $principal = $this->seal(['schema' => ImperatorRuntimePrincipalVersionContract::SCHEMA, 'principal_version_id' => $principalVersionId, 'principal_id' => $principalId, 'instance_id' => 'imperium-offline-interruption-demonstration', 'binding_id' => 'imperator-offline-binding', 'principal_generation' => 1, 'constitution_route' => 'FUTURE_INSTANCE_ROOT_ESTABLISHMENT', 'source_constitution_authority' => ['id' => 'offline-constitution-authority', 'digest' => str_repeat('1', 64), 'schema' => 'offline-authority/v1'], 'source_operator_root' => ['id' => 'offline-operator', 'digest' => str_repeat('2', 64), 'schema' => 'offline-operator/v1'], 'identity' => ['operator_id' => 'offline-operator', 'operator_identity_digest' => str_repeat('3', 64), 'imperator_subject_id' => $principalId, 'imperator_subject_digest' => str_repeat('4', 64)], 'authority_scope' => ['provider_binding_activation_authority' => true, 'outbound_email_authority' => false, 'credential_authority' => false, 'provider_execution_authority' => false, 'corridor_disposition_authority' => false], 'lifecycle' => ['constituted_at' => $at->modify('-2 minutes')->format(DATE_ATOM), 'effective_at' => $at->modify('-2 minutes')->format(DATE_ATOM), 'expires_at' => $at->modify('+1 day')->format(DATE_ATOM), 'prior_version' => null, 'superseding_version' => null, 'current_disposition' => null], 'status' => 'ACTIVE', 'credential_reference_persisted' => false, 'credential_secret_persisted' => false, 'serialized_capability_persisted' => false, 'sealed' => true]);
        $this->write($root.'/'.DeterministicTransitionCallerAuthorityIssuanceService::IMPERATOR_PRINCIPALS.'/'.$principalVersionId.'.json', $principal);
