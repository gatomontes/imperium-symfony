...
final readonly class SymfonyAiProfileElaborationCognitionGateway implements ProfileElaborationCognitionGateway
{
    ...
    public function elaborate(array $acceptance, array $authorization): array
    {
        ...
        [$type, $id] = match ($acceptance['schema'] ?? null) {
            'imperium.laboratorium-profile-derivation-commission-acceptance/v1' => ['profile-elaboration', $acceptance['acceptance_id'] ?? ''],
            'imperium.laboratorium-delegate-mission-profile-derivation-commission-disposition/v1' => ['delegate-profile-elaboration', $acceptance['disposition_id'] ?? ''],
            default => throw new \RuntimeException('L40_PROFILE_ELABORATION_COGNITION_INVALID'),
        };
        $content = $this->invoker->invoke('laboratorium', $type, (string) $id, 'laboratorium.alchemist', 'elaborate-profile', [$acceptance, $authorization], $prompt);
        if ('' === trim($content)) throw new \RuntimeException('L40_PROFILE_ELABORATION_COGNITION_INVALID');
