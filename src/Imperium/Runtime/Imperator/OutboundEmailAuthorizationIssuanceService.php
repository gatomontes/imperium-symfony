...
final readonly class OutboundEmailAuthorizationIssuanceService
{
    ...
    public function issue(string $decisionId, \DateTimeImmutable $issuedAt): array
    {
        ...
        $authorization = ['schema' => DeterministicOutboundEmailAuthorizationContract::SCHEMA, 'authorization_id' => $authorizationId, 'instance_id' => $request['instance_id'], 'source_decision' => ['id' => $decisionId, 'digest' => $decision['record_digest'], 'schema' => $decision['schema'], 'decision' => $decision['disposition'], 'decision_owner' => $decision['actor']], 'issuer' => ['actor_id' => 'imperator-development-root', 'office' => 'imperator', 'seat' => 'imperator', 'binding_id' => 'imperator-development-root', 'runtime_principal_id' => 'imperator-development-root'], 'holder' => $request['holder'], 'scope' => $request['scope'], 'provider_safety' => $request['provider_safety'], 'single_use' => true, 'exercisable' => true, 'consumed' => false, 'issued_at' => $issuedAt->format(DATE_ATOM), 'expires_at' => $authority['expires_at'], 'continuing_authority' => false, 'sealed' => true];
