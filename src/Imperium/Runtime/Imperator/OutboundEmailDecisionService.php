...
final readonly class OutboundEmailDecisionService
{
    ...
    public function decide(string $callerAuthorityId, string $requestId, string $disposition, string $rationale, string $limitations, \DateTimeImmutable $expiresAt, \DateTimeImmutable $decidedAt): array
    {
        ...
        $caller = (new DeterministicTransitionCallerAuthorityConsumer($this->root))->consume($callerAuthorityId, 'DECIDE_EXACT_OUTBOUND_EMAIL_REQUEST', ['id' => $requestId, 'digest' => $request['record_digest']], self::class, $decidedAt);
        ...
        $record = ['schema' => OutboundEmailAuthorizationIssuanceContract::DECISION_SCHEMA, 'decision_id' => $decisionId, 'instance_id' => $request['instance_id'], 'source_request' => ['id' => $requestId, 'digest' => $request['record_digest']], 'actor' => ['kind' => 'imperator', 'id' => $caller['authority']['principal']['principal_id']], 'disposition' => $disposition, 'rationale' => $rationale, 'limitations' => $limitations, 'issuance_authority' => $issuanceAuthority, 'decided_at' => $decidedAt->format(DATE_ATOM), 'expires_at' => $expiresAt->format(DATE_ATOM), 'external_action_performed' => false, 'sealed' => true];
