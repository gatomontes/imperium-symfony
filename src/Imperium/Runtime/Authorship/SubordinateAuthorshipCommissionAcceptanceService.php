...
final readonly class SubordinateAuthorshipCommissionAcceptanceService
{
    ...
    public function accept(
        ...
        [$role, $seat] = match ($office) {
            ...
            default => throw new \InvalidArgumentException(
                "A93_AUTHORSHIP_OFFICE_INVALID",
            ),
        };
        if (
            !preg_match(
                "/^subordinate-authorship-" . $office . '-[a-f0-9]{20}$/',
                $commissionId,
            )
        ) {
            throw new \InvalidArgumentException(
                "A94_SUBORDINATE_COMMISSION_ID_INVALID",
            );
        }
        if (
            !preg_match(
            ...
            throw new \InvalidArgumentException(
                "A95_RESIDENT_BINDING_ID_INVALID",
            );
        }
        $c = $this->read(
            $this->officeRoot .
                ...
                $commissionId .
                ".json",
            "A96_SUBORDINATE_COMMISSION_ABSENT",
        );
        if (
            !$this->digestMatches($c) ||
            "imperium.subordinate-persona-authorship-commission/v1" !==
                ($c["schema"] ?? null) ||
            $commissionId !== ($c["commission_id"] ?? null) ||
            $office !== ($c["office"] ?? null) ||
            $seat !== ($c["target_seat"] ?? null) ||
            "ISSUED_PENDING_RECIPIENT" !== ($c["status"] ?? null) ||
            true !== ($c["authorship_authority"] ?? null) ||
            null !== ($c["recipient_acceptance"] ?? null) ||
            true === ($c["persona_assembly_authority"] ?? null) ||
            true === ($c["persona_approval_authority"] ?? null) ||
            true === ($c["profile_approval_authority"] ?? null) ||
            true === ($c["spawning_authority"] ?? null) ||
            true === ($c["admission_authority"] ?? null) ||
            true === ($c["seat_binding_authority"] ?? null) ||
            true === ($c["execution_authority"] ?? null)
        ) {
            throw new \RuntimeException("A97_SUBORDINATE_COMMISSION_INVALID");
        }
        $b = $this->read(
            $this->officeRoot .
                "/" .
        ...
        if (
            ...
            true !== ($b["authorship_authority"] ?? null) ||
            true === ($b["execution_authority"] ?? null) ||
            ($c["instance_id"] ?? null) !== ($b["instance_id"] ?? null)
        ) {
            throw new \RuntimeException("A99_RESIDENT_BINDING_INVALID");
        }
        $specId = $c["persona_specification_id"] ?? null;
        $s = is_string($specId)
            ? $this->read(
                $this->specificationDirectory . "/" . $specId . ".json",
                "A100_SUBORDINATE_COMMISSION_CHAIN_INVALID",
            )
            : [];
        ...
        if (
            ...
            true !== ($s["sealed"] ?? null) ||
            "imperium.foundry-subordinate-construction-case/v1" !==
                ($case["schema"] ?? null) ||
            "OPEN_PENDING_PERSONA_SPECIFICATION" !==
                ($case["status"] ?? null) ||
            ...
            CanonicalJson::encode($c["persona_specification"] ?? null) !==
                CanonicalJson::encode($s["specification"] ?? null) ||
            CanonicalJson::encode($c["inherited_requirements"] ?? null) !==
        ...
        return $this->persist($office, $id, [
            ...
            "required_product" => $c["required_product"],
            "forbidden_authorship" => $c["forbidden_authorship"],
    ...
    private function persist(string $office, string $id, array $r): array
    {
        ...
        if (!is_dir($dir) && !mkdir($dir, 0770, true) && !is_dir($dir)) {
            throw new \RuntimeException(
                "Subordinate authorship acceptance directory cannot be created.",
            );
        }
        ...
        if (is_file($p)) {
            $old = $this->read(
                $p,
                "A101_SUBORDINATE_ACCEPTANCE_REPLAY_CONFLICT",
            );
        ...
        foreach (
            glob($dir . "/" . $office . "-subordinate-acceptance-*.json") ?: []
            as $ep
        ) {
            $old = $this->read(
                $ep,
            ...
            if ($r["commission_id"] === ($old["commission_id"] ?? null)) {
                throw new \RuntimeException(
                    "A102_SUBORDINATE_COMMISSION_ALREADY_DISPOSED",
                );
            }
        }
        ...
        if (
            ...
            @unlink($tmp);
            throw new \RuntimeException(
                "Subordinate authorship acceptance cannot be committed atomically.",
            );
        }
        return $r;
    }
