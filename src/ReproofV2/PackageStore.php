...
final class PackageStore
{
    ...
    {
        $reservation = json_decode((string) file_get_contents($directory.'/reservation.json'), true, flags: JSON_THROW_ON_ERROR);
    ...
    {
        $final = @file_get_contents($directory.'/finalized.json');
        if (false === $final) { throw new \RuntimeException('REPROOF_PACKAGE_INCOMPLETE'); }
        try {
            ...
            foreach (['receipt', 'candidate'] as $name) {
                $bytes = @file_get_contents($directory.'/'.$name.'.json');
                if (false === $bytes || hash('sha256', $bytes) !== $manifest[$name.'_digest']) { throw new \RuntimeException(); }
