...
The next model-bound Citadel transition is step 36, **runtime activation**. It remains unimplemented in this reconstruction checkpoint.
...
35. **Authorize activation of the exact bound Citadel Officer.** The Imperator decides against the exact generation-1 binding and complete Profile/model-access lineage. Only `AUTHORIZED` opens one single-use Conscription runtime-activation authority at `MODEL_BOUND_MANIFESTATION_ACTIVATION_AUTHORIZED_PENDING_RUNTIME_ACTIVATION`. Refusal, revision, alternative, clarification, and deferral remain sealed and non-authorizing. Runtime activation, operational use, tools, credentials, provider invocation, external action, and execution remain closed.
