...
18. **Conscription assembles the examination Delegate.** The occupied Recruiter mechanically consumes both examination-specific authorities and installs the complete sealed candidate content and scope—not merely its identity metadata—into generic Officer substrate version 0, explicitly classified as `DELEGATE`. This gives Senate the substantive operating posture, directives, contracts, limitations, and failure behavior it must examine while the sealed Manifestation remains examination-only. It is delivered to the Bailiff's stand intake at `EXAMINATION_MANIFESTATION_ASSEMBLED_DELIVERED_PENDING_SENATE_STAND_INTAKE`; no operational installation, stand admission, examination, approval, deployment, or execution occurs.
...
33. **Conscription assembles the operational Legate.** Consume the exact qualification's assembly authority and mechanically combine the custody-bound Persona, qualified operational Profile, and identity- and authority-neutral generic Officer version-0 substrate, explicitly classified as `LEGATE`. Seal `OPERATIONAL_MANIFESTATION_ASSEMBLED_PENDING_SEAT_BINDING` with one bounded Seat-binding authority, while the Manifestation remains unbound and prohibited from operational use. No tool access, credentials, external action, deployment, custody transfer, or execution authority exists.
...
35. **Authorize activation of the exact bound Legate.** The Imperator decides against the exact generation-1 binding and complete Profile/model-access lineage. Only `AUTHORIZED` opens one single-use Conscription runtime-activation authority at `MODEL_BOUND_CITADEL_LEGATE_ACTIVATION_AUTHORIZED_PENDING_RUNTIME_ACTIVATION`. Refusal, revision, alternative, clarification, and deferral remain sealed and non-authorizing. Runtime activation, operational use, tools, credentials, provider invocation, external action, and execution remain closed.

36. **Activate the exact Legate runtime mechanically.** Conscription consumes the exact Step 35 authority, proves the generation-1 binding remains the sole current occupancy for the immutable Seat, and revalidates the complete lineage and unexpired access attestation. The Legate reaches `MODEL_BOUND_CITADEL_LEGATE_RUNTIME_ACTIVE_PENDING_GOVERNED_COMMISSION` with commission intake available, but no cognition, provider invocation, credential, tool, external-action, execution, or continuing-turn authority.

37. **Issue one exact governed commission.** An actively occupied caller whose own binding explicitly authorizes commissions to the target Seat seals the exact task, purpose, inputs, evidence requirements, constraints, output contract, and stop conditions. The commission reaches `CITADEL_LEGATE_GOVERNED_COMMISSION_ISSUED_PENDING_LEGATE_ACCEPTANCE` with one single-use acceptance authority addressed only to the target Legate. Issuance grants no acceptance, cognition, provider invocation, credential, tool, external-action, execution, or continuing-turn authority.

38. **The exact target Legate accepts or refuses the commission.** The target revalidates the commission, activation, exact current binding, unexpired model access, immutable contract, and single-use acceptance authority. Acceptance binds the Legate at `CITADEL_LEGATE_GOVERNED_COMMISSION_ACCEPTED_PENDING_COGNITION_TURN_AUTHORIZATION`; refusal reaches `CITADEL_LEGATE_GOVERNED_COMMISSION_REFUSED_NO_AUTHORITY`. Neither branch authorizes cognition, provider invocation, credentials, tools, external action, operational use, execution, or a continuing turn.

39. **The original commissioner decides one bounded cognition turn.** The exact occupied issuer revalidates the accepted commission, target Legate, runtime activation, current sole occupancy, unexpired model access, immutable contract, and complete digest chain. Only `AUTHORIZED` creates a target-bound, single-use, maximum-one-turn authority expiring within fifteen minutes at `CITADEL_LEGATE_COGNITION_TURN_AUTHORIZED_PENDING_PROVIDER_INVOCATION_ACTIVATION`. Refusal, deferral, and revocation grant nothing. Authorization does not activate or invoke the provider, release credentials, perform cognition, permit tools or external action, execute, or create a continuing turn.
