# Owner setup and resume — readiness v2

Use only the fresh package named in `docs/local-isolation-readiness-audit.json`.
...
$audit=Get-Content 'E:\htdocs\imperium\docs\local-isolation-readiness-audit.json' -Raw|ConvertFrom-Json
...
Planned changes are a fresh `C:\ProgramData\Imperium` and its code, state, PHP,
