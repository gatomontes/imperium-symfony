...
Twenty-nine direct platform-bound agent definitions remain across the main pre-Delegate governance clusters after the operational and Foundry definitions' removal. The isolated sortie runtime adds one more direct agent and a second `%env(DEEPSEEK_API_KEY)%` platform definition. The system-wide inventory is therefore 30 remaining agents across eight clusters and two credential-bearing platforms. The final evidence gate is not closed.
...
Credential-boundary Batch 8 migrates Foundry's specification, revision, ordinary-review, and adversarial-review cognition. Each gateway now locates one exact pre-staged governance claim bound to its native authority, ordered input digest, Seat, and purpose; reserves the provider journal before credential resolution; and prohibits replay or cross-stage substitution. The Foundry agent definitions and direct injections are removed. The inventory is 30; the system-wide gate remains open.
