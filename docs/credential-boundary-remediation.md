...
Twenty-seven direct platform-bound agent definitions remain across the main pre-Delegate governance clusters after the operational, Foundry, and resident-requirements definitions' removal. The isolated sortie runtime adds one more direct agent and a second `%env(DEEPSEEK_API_KEY)%` platform definition. The system-wide inventory is therefore 28 remaining agents across seven clusters and two credential-bearing platforms. The final evidence gate is not closed.
