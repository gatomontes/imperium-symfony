...
```powershell
Set-Location E:\htdocs\imperium-citadel-native-lineage
python -B tools/citadel_native_lineage.py collect --root E:\htdocs\imperium --output E:\htdocs\citadel-native-lineage-public-next
```

Expected native exit **2** means collection completed with explicit authority blockers; it never means live readiness. Exit **1** means a refused/malformed/unsafe request or I/O failure. Do not equate PowerShell's host exit status with the native Python `$LASTEXITCODE`; retain the native code.
...
```powershell
python -B tools/citadel_native_lineage.py verify E:\htdocs\citadel-native-lineage-public-next\collection.json
```

Expected exit **0** verifies retained original-byte hashes, supported record digest checks, bounded reference membership and the recomputed refusing assessment. It does not independently confirm unavailable observations, collection time/root, completeness, custody, source execution or currentness. Exit **1** refuses a corrupt, inconsistent, unsafe or unsupported packet. No projection format is accepted: original bytes are base64 with their own SHA-256; altered/redacted bytes cannot borrow that identity.

Limits: 1 MiB per source file, 8 MiB aggregate, 32 source records, 4,096 directory names for the one exact summons lookup, JSON depth 64. No recursive runtime scan; the directory lookup reads names only until exactly one candidate is found. Symlinks/reparse points, hard links, traversal, UNC/network roots and nonregular files refuse. Windows handle validation checks the final path and reparse/file attributes before reading and denies write/delete sharing; POSIX uses directory handles with O_NOFOLLOW. The local collection is not an independently timestamped filesystem snapshot or protection against an administrator replacing an entire trusted installation. Before/after source and content observations belong to the review packet.

The tool has no application boot, provider, credential, signing, enrollment or authority-resolution dependency. Its installation argument is solely a read-only collection root. It never enters production FormationInstitution or changes trusted-root DI. Public errors use fixed codes, not source contents or exception paths. Python canonicalization supports nonnumeric object keys and signed 64-bit integers, rejecting floats and ambiguous JSON; PHP CanonicalJson cross-checks the actual collected corpus in the review proof.
...
5. Owner/custodian: named installation custodian, public custody references/time and existing signing interface; formation Citadel ID, raw public Ed25519 key, independently confirmed fingerprint, CITADEL_MISSION_FORMATION competence, validity/revocation and enrollment receipt. Keep unsigned unknown values null. Do not read a formation journal or reuse ProtectedMission competence.
6. Qualified institutional actors and owner: genuine candidate admission, suitability, derived Profile, four Senate findings/reconciliation, exact Profile approval, qualification and separate Castellan/formation Locksmith appointment terms (`candidate`, `scope`, `seat`, `generation`). Authentic role-limited formation delegation remains separate from structural occupancy evidence. No signing/import command is proposed with invented identities.
