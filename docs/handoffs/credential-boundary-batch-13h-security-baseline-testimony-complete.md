...
The Security completion service requires the unconsumed single-use authority, invokes the shared claim-bound broker path, seals the exact answer, and records the authority as consumed. The resulting immutable fourth turn stops at `SECURITY_BASELINE_TESTIMONY_SEALED_PENDING_BASELINE_AGGREGATION`.
...
Batch 13I migrates the four jurisdictional question-authoring agents to exact claim-bound authorities and removes their direct definitions, reducing inventory from 15 to 11. The next lifecycle step is mechanical four-turn baseline aggregation before any fresh-instance or pressure trial.
