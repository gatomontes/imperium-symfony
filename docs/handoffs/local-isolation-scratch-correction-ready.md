...
owner runbook/audit/ledger, fresh packet/manifest and all affected ceremony,
installer, checker, probe, validator and test sources. Read applicable repository
and ancestor AGENTS.md and the current selection in flow and todo.

Verify E:/htdocs/imperium, origin gatomontes/imperium-symfony, branch, full
HEAD/tree, worktree and refs. Preserve codex/local-isolation-measurement-readiness.
Confirm 3e61c0c283aca4fbdd51179405cd7fcc17be3fcf exists in its local lineage;
inspect and preserve later commits, the review assembler and dirty user work.
Do not reconstruct the unpublished implementation from main.

Create or safely resume codex/local-isolation-scratch-correction from the latest
verified local implementation descendant. A read-only fetch is authorized.
Record the exact main merge containing this selection; inspect changes since
837f2731e705d9aa9737d947b481b57ff0b494f8 and integrate the documentation locally
into the NEW branch. Reconcile documentary overlap by keeping this selection
current and old audits intact. Assess unexpected code changes/conflicts; never
reset, force, silently stash, or merge unpublished implementation into main.

Complete S0–S2 sequentially with separate commits. Prove the real ceremony under
equivalent Windows ACLs, preserve reference protection, cover cleanup/recovery,
and integrate scratch into startup/readiness evidence. A synthetic permission
result cannot satisfy the positive native proof.

Commit code/tests before final audit, run meaningful focused/full/PowerShell
verification, and rebuild under a new package identity. Retain all earlier
packages, hashes, tests, branches and attempt markers.

Return exact source/integration/test/final identifiers, counts/skips, native token
and ACL proof, changed-test map, package path/digest, updated owner commands,
independent-review Markdown and derivative manifest paths. Prepare everything
safe before requesting any missing human action.

No real installation/accounts, keys/passwords in agent custody, journal reset,
